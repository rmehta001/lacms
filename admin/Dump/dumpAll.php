...
$quStrTypes = "SELECT * FROM TYPES WHERE TYPE <=13";
...
while ($rowTypes = mysqli_fetch_object($quTypes)) {
	...
	while ($rowLocs = mysqli_fetch_object($quLocs)) {
		...
		fwrite ($dumpFile, "$html_head \n \n");
       		$quStrGetAds = "SELECT * FROM ((CLASS INNER JOIN `GROUP` ON CLASS.CLI=`GROUP`.GRID) INNER JOIN LOC ON CLASS.LOC=LOC.LOCID) INNER JOIN USERS ON CLASS.UID=USERS.UID WHERE TYPE=$rowTypes->TYPE AND STATUS='ACT' AND LOC=$rowLocs->LOCID ORDER BY SPORDER ASC,  STATUS_SALE ASC, RAND()";
		$quGetAds = mysqli_query($dbh, $quStrGetAds) or die("error,  locations query failed, dump aborted");
		$numGetAds = mysqli_num_rows($quGetAds);
