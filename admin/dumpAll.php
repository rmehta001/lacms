...
mysqli_query("LOCK TABLE CLASS READ");
mysqliquery("BACKUP TABLE CLASS TO '/usr/home/eboyer/lacmsBACKUP/$nowYYYYMMDD/'");
mysqli_query("UNLOCK TABLES");

mysqli_query("LOCK TABLE `GROUP` READ");
mysqli_query("BACKUP TABLE `GROUP` TO '/usr/home/eboyer/lacmsBACKUP/$nowYYYYMMDD/'");
mysqli_query("UNLOCK TABLES");

mysqli_query("LOCK TABLE `USERS` READ");
mysqli_query("BACKUP TABLE `USERS` TO '/usr/home/eboyer/lacmsBACKUP/$nowYYYYMMDD/'");
mysqli_query("UNLOCK TABLES");

mysqli_query("LOCK TABLE PICTURE READ");
mysqli_query($dbh, "BACKUP TABLE PICTURE TO '/usr/home/eboyer/lacmsBACKUP/$nowYYYYMMDD/'");
mysqli_query($dbh, "UNLOCK TABLES");

mysqli_query($dbh, "LOCK TABLE LANDLORD READ");
mysqli_query($dbh, "BACKUP TABLE LANDLORD TO '/usr/home/eboyer/lacmsBACKUP/$nowYYYYMMDD/'");
mysqli_query($dbh, "UNLOCK TABLES");

mysqli_query($dbh, "LOCK TABLE TYPES READ");
mysqli_query($dbh, "BACKUP TABLE TYPES TO '/usr/home/eboyer/lacmsBACKUP/$nowYYYYMMDD/'");
mysqli_query($dbh, "UNLOCK TABLES");
...
$quStrTypes = "SELECT * FROM TYPES WHERE TYPE <=7";
...
while ($rowTypes = mysqli_fetch_object($quTypes)) {
	...
	$quStrGetAds = "SELECT * FROM ((CLASS INNER JOIN `GROUP` ON CLASS.CLI=`GROUP`.GRID) INNER JOIN LOC ON CLASS.LOC=LOC.LOCID) INNER JOIN USERS ON CLASS.UID=USERS.UID WHERE TYPE=$rowTypes->TYPE AND STATUS='ACT' $locations ORDER BY LOCNAME, SPORDER ASC, STATUS_SALE ASC, RAND()";
...
while ($rowTypes = mysqli_fetch_object($quTypes)) {
	...
	$colCount = 1;
	$quStrLocs = "SELECT * FROM LOC $locations_loc ORDER BY STATE, LOCNAME";
	$quLocs = mysqli_query($dbh, $quStrLocs) or die ("error, Locs query faild, dunp aborted");
	while ($rowLocs = mysqli_fetch_object($quLocs)) {
...
// original file name path	$cell = "<TD width='225'><font size='-1'><a href=\"./$rowTypes->ALTNAME-$rowLocs->LOCNAME.htm\">$titleCased</a></font></TD> \n"; //

				if ($rowTypes->TYPE =="2") {

		$cell = "<TD width='225'><font size='-1'><a href=\"https://www.BostonApartments.com/cli-class.php?TYPE=$rowTypes->TYPE&LOC=$rowLocs->LOCID,0&template=ba-sales&sig=yes&usig=on&htype=ba-sales&tablewidth=930\">$titleCased</a></font></TD> \n"; 

				} elseif ($rowTypes->TYPE =="3") {

		$cell = "<TD width='225'><font size='-1'><a href=\"https://www.BostonApartments.com/cli-class.php?TYPE=$rowTypes->TYPE&LOC=$rowLocs->LOCID,0&template=ba-sales&sig=yes&usig=on&htype=bacomm&tablewidth=930\">$titleCased</a></font></TD> \n"; 

				} elseif ($rowTypes->TYPE =="4") {

		$cell = "<TD width='225'><font size='-1'><a href=\"https://www.BostonApartments.com/cli-class.php?TYPE=$rowTypes->TYPE&LOC=$rowLocs->LOCID,0&template=ba&sig=yes&usig=on&htype=bacomm&tablewidth=930\">$titleCased</a></font></TD> \n"; 

				} elseif ($rowTypes->TYPE =="10") {

		$cell = "<TD width='225'><font size='-1'><a href=\"https://www.BostonApartments.com/cli-class.php?TYPE=$rowTypes->TYPE&LOC=$rowLocs->LOCID,0&template=ba&sig=yes&usig=on&htype=ba-busop&tablewidth=930\">$titleCased</a></font></TD> \n"; 

				} elseif ($rowTypes->TYPE =="11") {

		$cell = "<TD width='225'><font size='-1'><a href=\"https://www.BostonApartments.com/cli-class.php?TYPE=$rowTypes->TYPE&LOC=$rowLocs->LOCID,0&template=ba&sig=yes&usig=on&htype=ba-seniorr&tablewidth=930\">$titleCased</a></font></TD> \n"; 

				} elseif ($rowTypes->TYPE =="12") {

		$cell = "<TD width='225'><font size='-1'><a href=\"https://www.BostonApartments.com/cli-class.php?TYPE=$rowTypes->TYPE&LOC=$rowLocs->LOCID,0&template=ba-sales&sig=yes&usig=on&htype=ba-seniors&tablewidth=930\">$titleCased</a></font></TD> \n"; 

				} elseif ($rowTypes->TYPE =="13") {

		$cell = "<TD width='225'><font size='-1'><a href=\"https://www.BostonApartments.com/cli-class.php?TYPE=$rowTypes->TYPE&LOC=$rowLocs->LOCID,0&template=ba-sales&sig=yes&usig=on&htype=ba-bankowned&tablewidth=930\">$titleCased</a></font></TD> \n"; 


				} else {

		$cell = "<TD width='225'><font size='-1'><a href=\"https://www.BostonApartments.com/cli-class.php?TYPE=$rowTypes->TYPE&LOC=$rowLocs->LOCID,0&template=ba&sig=yes&usig=on&htype=baapt&tablewidth=930\">$titleCased</a></font></TD> \n"; 
				}

	}else {

			$cell = "<TD width='225'><font size='-1'>$titleCased</font></TD> \n";
...
/*  BAD MEMORY EATER -- THIS IS MOVED TO THE NEW FUNCTION format_ad_2() 
	...
	$readFile = eregi_replace ("<A HREF=\"", "<A HREF=\"https://www.BostonApartments.com/", $readFile);
	$readFile = eregi_replace ("<A HREF=\"https://www.BostonApartments.com/https://www.BostonApartments.com/", "<A HREF=\"https://www.BostonApartments.com/", $readFile);
	$readFile = eregi_replace ("<A HREF=\"https://www.BostonApartments.com/mailto:", "<A HREF=\"mailto:", $readFile);
	$readFile = eregi_replace ("https://www.BostonApartments.com/http:", "http:", $readFile);
	fclose ($fp);
	$fileToRead = "$liveDataDir$rowTypes->FNAME" . "2.htm";
