...
$preFilters[9] = ($_POST['status_vacant'][0]) ? array($_POST['status_vacant'], "STATUS_VACANT", "=", "Vacant") : "";
		...
		$preFilters[121] = ($_POST['status_pending'][0]) ? array($_POST['status_pending'], "STATUS_PENDING", "=", "Pending") : "";

		$preFilters[122] = array($_POST['modby'], "MODBY", "=", "Mod by");

		$preFilters[123] = ($_POST['street_num_begin'][0]) ? array($_POST['street_num_begin'], "CLASS.STREET_NUM", " >= ", "Street Number Begin") : "";
			$preFilters[124] = ($_POST['street_num_end'][0]) ? array($_POST['street_num_end'], "CLASS.STREET_NUM", " <= ", "Street Number End") : "";
		...
		$preFilters[127] = array($_POST['keyword'], "CLASS.BODY"," LIKE ", "keyword");

		$preFilters[128] = array($_POST['amenities_bikeroom'], "CLASS.AMENITIES_BIKEROOM","=", "Amenities: Bike Room ");
		$preFilters[129] = array($_POST['features_cabletv'], "CLASS.FEATURES_CABLETV","=", "Features: Cable TV");
		$preFilters[130] = array($_POST['amenities_playground'], "CLASS.AMENITIES_PLAYGROUND","=", "Amenities: Playground ");
		$preFilters[131] = array($_POST['amenities_basketball'], "CLASS.AMENITIES_BASKETBALL","=", "Amenities: Basketball ");
		$preFilters[132] = array($_POST['features_stainless'], "CLASS.FEATURES_STAINLESS","=", "Features: Stainless");
		$preFilters[134] = array($_POST['features_granitec'], "CLASS.FEATURES_GRANITEC","=", "Features: Granite Countertop");
		$preFilters[135] = array($_POST['features_baywindows'], "CLASS.FEATURES_BAYWINDOWS","=", "Features: Bay Windows");
		$preFilters[136] = array($_POST['amenities_bbq'], "CLASS.AMENITIES_BBQ","=", "Amenities: BBQ ");
		$preFilters[137] = array($_POST['amenities_zipcar'], "CLASS.AMENITIES_ZIPCAR","=", "Amenities: Zip Car ");
		$preFilters[138] = array($_POST['features_intercom'], "CLASS.FEATURES_INTERCOM","=", "Features: Intercom");
		$preFilters[139] = array($_POST['features_ceilingfan'], "CLASS.FEATURES_CEILINGFAN","=", "Features: Ceiling Fan");
		$preFilters[140] = array($_POST['nopic'], "CLASS.PIC","<", "Has No Pics" );
...
//                                                $filters[$i] = " ($preFilter[1]$preFilter[2]'%$preFilter[0]%') ";
	...
	if($sourcemls=="$grid") {
		...
		switch ($numFilters) {
			...
			$WHERE = " WHERE (CLI='$grid' OR CLI='1075') $userFilterStr $activeFilterStr $availFilterStr";
...
if ($disData=="landlords" || $disData2=="landlords" || $disData3=="landlords") {
	...
	$quStrGetLandlordCount = "select count(LID) as landlords_count from $landlords_table_set $landlords_where  AND GRID=$grid $landlords_order_by";
