...
//BEGIN adlEdit.php //
    ...
    $quStrGetCLI = "select `CLI`, `CSOURCE`,`LOCKED` from `CLASS` where `CID`=".$_POST['cid'];
...
if (($testLOCKED=="1" AND ($_POST['cid'])) AND $user_level<"4") {

$msg = "Listing Locked by Admin - Ask Office Admin for Help";
...
if ($isAdmin OR ($user_level >="4")) {
		$UPDATESTR = "UPDATE CLASS SET `MOD`='$now', MODBY='$handle', AVAIL='$avail', LEASE_EXPIRE='$lease_expire', `DATEONMARKET`='$dateonmarket', BODY='$body', AGENCY_HEADERS='$AH', LOCKED='$LOCKED', ";
} else {
