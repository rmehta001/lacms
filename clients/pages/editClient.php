...
?>
	...
	<nobr>
	<a href="<?php echo "$PHP_SELF?op=hot_list_add&item_type=3&item_id=$clid&return_page=$op&return_page_rid=$clid";?>"><img border="0" hspace="0" vspace="0" width="96" height="23" src="../assets/images/addToHotList.jpg"></a>  &nbsp; | &nbsp; <a href="<?php echo "$PHP_SELF?op=listings&client_id_filter=$rowGetClient->CLID";?>" target="_<?php if(isset($rowGetClient)) echo $rowGetClient->CLID;?>"><img border="0" hspace="0" vspace="0" width="19" height="19" src="../assets/images/matchlistings.gif" TITLE="Match Client to Listings" ALT="Match Client to Listings"> Match Client to Listings</a> &nbsp; | &nbsp; <A HREF="<?php echo "$PHP_SELF?op=editClientReassign&clid=$rowGetClient->CLID&fname=$rowGetClient->NAME_FIRST&lname=$rowGetClient->NAME_LAST";?>"><FONT SIZE="-2"><img border="0" hspace="0" vspace="0" width="16" height="16" src="../assets/images/client-reassign.gif" TITLE="Reassign Client" ALT="Reassign Client"> Reassign Client</FONT></A> 

| <a href="<?php if(isset($rowGetClient))  echo "$PHP_SELF?op=showingsClient&clid=$rowGetClient->CLID";?>" TITLE="Showing History for <?php if(isset($rowGetClient)) echo $rowGetClient->NAME_FIRST.$rowGetClient->NAME_LAST;?>" target="_sh<?php if(isset($rowGetClient)) echo $rowGetClient->CLID;?>"><img border="0" hspace="0" vspace="0" width="16" height="16" src="../assets/images/showings-history.jpg" TITLE="Showing History" ALT="Showing History"> Showing History</A>

 | <a href="<?php if(isset($rowGetClient)) echo "$PHP_SELF?op=createshowingentry&clid=$rowGetClient->CLID";?>" TITLE="Add Showing History Entry for <?php if(isset($rowGetClient)) echo $rowGetClient->NAME_FIRST.$rowGetClient->NAME_LAST;?>" target="_sh<?php if(isset($rowGetClient)) echo $rowGetClient->CLID;?>"><img border="0" hspace="0" vspace="0" width="16" height="16" src="../assets/images/showings.jpg" TITLE="Showing History" ALT="Showing History"> Add Showing Entry</A>
...
<NOBR><FONT SIZE="+1">Client Information:</FONT> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Status: <select name="CLIENT_STATUS2" ID="CLIENT_STATUS2" STYLE="Background-Color : #FFFFFF">
					...
					<?php } ?>
				</select>  &nbsp; <?php if(isset($rowGetClient)) if ($rowGetClient->STATUS_CLIENT==1) {echo "<B>";}?>Active<?php if(isset($rowGetClient)) if ($rowGetClient->STATUS_CLIENT==1) {echo "</B>";}?><img border="0" hspace="2" vspace="0" width="16" height="16" src="../assets/images/client-active.jpg"><input type="radio" name="status_client" value="1" <?php if(isset($rowGetClient)) if ($rowGetClient->STATUS_CLIENT==1) {echo "checked";}?>> <?php if(isset($rowGetClient)) if ($rowGetClient->STATUS_CLIENT==2) {echo "<B>";}?>Inactive<?php if(isset($rowGetClient)) if ($rowGetClient->STATUS_CLIENT==2) {echo "</B>";}?><img border="0" hspace="1" vspace="0" width="16" height="16" src="../assets/images/client-inactive.jpg"><input type="radio" name="status_client" value="2" <?php if(isset($rowGetClient)) if ($rowGetClient->STATUS_CLIENT==2) {echo "checked";}?>> 
...
<?php
	if(isset($rowGetClient)) if ($rowGetClient->TYPE_PREF=="2" || $rowGetClient->TYPE_PREF=="3" || $rowGetClient->TYPE_PREF=="5" || $rowGetClient->TYPE_PREF=="6" || $rowGetClient->TYPE_PREF=="7" || $rowGetClient->TYPE_PREF=="8" || $rowGetClient->TYPE_PREF=="9" || $rowGetClient->TYPE_PREF=="10" || $rowGetClient->TYPE_PREF=="11" || $rowGetClient->TYPE_PREF=="12" || $rowGetClient->TYPE_PREF=="13") {
...
<select name="mi_month" STYLE="Background-Color : #FFFFFF">
						<option value="00" <?php if ($mi_month=='00') { echo "selected";}?>>--</option>
...
<NOBR>Created: &nbsp;<?php if(isset($rowGetClient)) echo $rowGetClient->DATE_CREATED;?> by <?php 

if(isset($rowGetClient)) if($rowGetClient->CREATED_BY)
...
?></NOBR>
<NOBR>Last Mod: <?php if(isset($rowGetClient)) echo $rowGetClient->DATE_MODIFIED;?> by <?php 
if(isset($rowGetClient)) 
...
?></NOBR>
<BR><NOBR>
Share Client with WHOLE OFFICE? <input type="checkbox" name="public" value="1" <?php if(isset($rowGetClient)) if ($rowGetClient->PUBLIC != "0") { echo " checked "; } ?>>



<?php 
if(isset($rowGetClient)) if ($rowGetClient->SHARE_WITH) {
...
<NOBR>					
		Subscribed to Daily Emails &amp; Newsletters: <input type="checkbox" name="newsletter_subscribe" <?php if(isset($rowGetClient)) if ($rowGetClient->NEWSLETTER_SUBSCRIBE=='2') { echo checked;} ?> value="2">

&nbsp;&nbsp;&nbsp;&nbsp; 
<?php
	echo "<A HREF=\"$PHP_SELF?op=mail_client_history&clid=$rowGetClient->CLID\" target=\"_new\"><IMG src=../images/icons/email.gif BORDER=0 HEIGHT=15 WIDTH=22> Listing Email History</A>";
?>
			...
			if(isset($rowGetClient)) if ($rowGetClient->TYPE_PREF== $rowTypes->TYPE) { echo " selected "; }
...
<input type="checkbox" name="client_leadsafe" value="1" <?php if ($rowGetClient->LEADSAFE) { echo " checked "; } ?>><br></td>
...
<div class="controltext"><NOBR>Location preference(s):</NOBR></div><select name="loc_pref[]" multiple SIZE=8 STYLE="Background-Color : #FFFFFF">
	...
	<option value="<?php echo $rowFavLocs->LOCID;?>" <?php if (isset($rowGetAd)) if ($rowGetAd->LOC==$rowFavLocs->LOCID) {echo " selected"; $locSeled = true; }?>><?php if (isset($rowFavLocs)) echo $rowFavLocs->LOCNAME;?></option>
	<?php } ?>
<option value="0">--------------------</option>
				
				<?php while ($rowLocs = mysqli_fetch_object($quLocs)) { ?>
				<option value="<?php if (isset ($rowLocs)) echo $rowLocs->LOCID;?>" <?php if (in_array($rowLocs->LOCID, $loc_pref)) { echo " selected "; }?>><?php echo $rowLocs->LOCNAME;?></option>
				<?php } ?>
				</select>
