...
class scraper
{
    ...
    function startScraping()
    {
        ...
        {
            $insert = mysqli_query($dbh, "INSERT INTO `emaillist` (`emailadd`) VALUES ('$curEmail')");
        ...
        // Mark the page done
        $insert = mysqli_query($dbh, "INSERT INTO `finishedurls` (`urlname`) VALUES ('".$this->startURL."')");
        ...
        {
            $insert = mysqli_query($dbh, "INSERT INTO `workingurls` (`urlname`) VALUES ('$curURL')");
            if($insert){$insertURLCount++;}
        }
        
        echo 'URLs found: '.number_format($insertURLCount).PHP_EOL;

        $getURL = mysqli_fetch_assoc(mysqli_query($dbh, "SELECT `urlname` FROM `workingurls` ORDER BY RAND() LIMIT 1"));
        $remove = mysqli_query($dbh, "DELETE FROM `workingurls` WHERE `urlname`='$getURL[urlname]' LIMIT 1");
