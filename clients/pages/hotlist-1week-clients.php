...
<CENTER>
		<B>LAST 7 DAY'S NEW CLIENT LEADS</B><br>
<HR>

<P>
<FONT SIZE="-1"><a href="<?php echo "$PHP_SELF?op=hotlist-3days-clients";?>">Last 3 Day's New Leads - Click Here</A><BR></FONT>
<P>
...
<FONT style="font-size:14px;"><B>LAST 7 DAY'S NEW CLIENT LEADS:</B></FONT><BR>
<TABLE BORDER=0 WIDTH="100%">

<?php 	
$quStrGet7daysClients = "select * from CLIENTS where (DATE_CREATED BETWEEN DATE_SUB(CURDATE() ,INTERVAL 7 DAY) AND CURDATE()) and GRID='$grid' AND UID='$uid' ORDER BY NAME_LAST";
	$quGet7daysClients = mysqli_query($dbh, $quStrGet7daysClients) or die (mysqli_error($dbh));
	while ($rowGet7daysClients = mysqli_fetch_object($quGet7daysClients)) {




	$quStrGetClientH = "select * from CLIENTS where CLID='$rowGet7daysClients->CLID' and UID='$uid' and GRID='$grid' LIMIT 1";
...
<TR><TD WIDTH="300">
		...
		<a href="<?php echo "$PHP_SELF?op=editClient&clid=$rowGet7daysClients->CLID";?>" target="_<?php echo $rowGet7daysClients->CLID;?>"><img border=0 src="../images/icons/edit.gif" alt="edit"><?php echo "$rowGet7daysClients->NAME_FIRST $rowGet7daysClients->NAME_LAST";?></a>
...
<?php if ($rowGet7daysClients->UID=="$uid") {?>
