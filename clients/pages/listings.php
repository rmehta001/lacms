...
function togglemenu() {
	...
	var text = document.getElementById("displayMenu");
        if(ele.getAttribute("style").includes("background")) {
                ele.setAttribute("style", "display: none;");
		text.innerHTML = "Show Menu";
  	}
	else {
                ele.setAttribute("style", "background-color:#75B9FF;");
		text.innerHTML = "Hide Menu";
	}
}
</script>
...
if ($_SESSION["listsearchshow"] != "n") {
    $listsearchs1 = $_SESSION["listsearch"];
} else {
    $listsearchs1 = "none";
}
$filter_string_display = "";
?>

<?php
$quStrGetViews = "SELECT * FROM VIEWS WHERE GRID=$grid OR PUBLIC=1 ORDER BY NAME";
//echo $quStrGetViews;
$quGetViews = mysqli_query($GLOBALS['dbh'], $quStrGetViews) or die($quStrGetViews);
$quStrGetView = "SELECT * FROM VIEWS WHERE ID=$vid AND (GRID='$grid' OR PUBLIC=1)";
$quGetView = mysqli_query($GLOBALS['dbh'], $quStrGetView);
$rowGetView = mysqli_fetch_array($quGetView);

$quStrGetUsers = "select * from USERS where `GROUP`=$grid ORDER BY `HANDLE`";
$quGetUsers = mysqli_query($GLOBALS['dbh'], $quStrGetUsers) or die(mysqli_error());

// AGENCY_HEADERS
if ($vid == 9 && $_SESSION["agcy"] > 0) {
...
$cols = explode(",", $rowGetView['COLUMNS']);
...
$quLimitCount = mysqli_query($GLOBALS['dbh'], $quStrLimitCount) or die(dieNice("Sorry, I could not find an exact match to your search and pagenate your listings. Please click \"Listings\" in the menu and clear and try another search. You can use the \"Quick Search\" to find listings that you may not be able to get an exact match . $WHERE", "E-200"));
...
if (isset($_GET['client_id_filter'])) {
    ...
    $clientFilterQ = mysqli_query($GLOBALS['dbh'], $clientFilterStr);
...
if (isset($_POST['filterChange'])) {
    ...
    //barkha
    $preFilters_str[1] = isset($_POST['type'][0]) ? array($_POST['type'], "CLASS.TYPE", "=", "Type") : "";
    $switch_remember['type'] = $_POST['type'][0];
    //barkha
    $preFilters_str[2] = array($_POST['loc'], "LOC.LOCID", "=", "Location");
    $switch_remember['loc'] = $_POST['loc'];
    if (isset($_POST['priceStart'][0])) //barkha
    {
    ...
    if (isset($_POST['priceEnd'][0])) //barkha
    {
    ...
    $preFilters_str[8] = isset($_POST['status'][0]) ? array($_POST['status'], "CLASS.STATUS", "=", "Advertising") : "";
    $switch_remember['status'] = $_POST['status'][0];
    $preFilters_str[9] = isset($_POST['status_vacant'][0]) ? array($_POST['status_vacant'], "STATUS_VACANT", "=", "Vacant") : "";
    $switch_remember['status_vacant'] = $_POST['status_vacant'][0];
    $preFilters_str[10] = isset($_POST['status_active'][0]) ? array($_POST['status_active'], "STATUS_ACTIVE", "=", "Available") : "";
    ...
    $switch_remember['bbbmonth'] = isset($_POST['bbbmonth']) ? $_POST['bbbmonth'] : ""; //barkha
    $switch_remember['bbbday'] = isset($_POST['bbbday']) ? $_POST['bbbday'] : ""; //barkha
    $switch_remember['bbbyear'] = isset($_POST['bbbyear']) ? $_POST['bbbyear'] : ""; //barkha
    $preFilters_str[12] = ($end_date) ? array($end_date, "CLASS.AVAIL", "=<", "Available end date") : "";
    $switch_remember['bbemonth'] = isset($_POST['bbemonth']) ? $_POST['bbemonth'] : ""; //barkha
    $switch_remember['bbeday'] = isset($_POST['bbeday']) ? $_POST['bbeday'] : ""; //barkha
    $switch_remember['bbeyear'] = isset($_POST['bbeyear']) ? $_POST['bbeyear'] : ""; //barkha
    ...
    $preFilters_str[16] = isset($_POST['terms'][0]) ? array($_POST['terms'], "CLASS.TERMS", "=", "Lease Terms") : "";
    ...
    $preFilters_str[18] = isset($_POST['tenant_fee'][0]) ? array($_POST['tenant_fee'], "CLASS.TENANT_FEE", "=", "Tenant Fee") : "";
    $switch_remember['tenant_fee'] = $_POST['tenant_fee'][0];
    $preFilters_str[19] = isset($_POST['landlord_fee'][0]) ? array($_POST['landlord_fee'], "CLASS.LANDLORD_FEE", "=", "Landlord Fee") : "";
    $switch_remember['landlord_fee'] = $_POST['landlord_fee'][0];
    $preFilters_str[20] = isset($_POST['payment_first'][0]) ? array($_POST['payment_first'], "CLASS.PAYMENT_FIRST", "=", "First Month Payment") : "";
    $switch_remember['payment_first'] = $_POST['payment_first'][0];
    $preFilters_str[21] = isset($_POST['payment_last'][0]) ? array($_POST['payment_last'], "CLASS.PAYMENT_LAST", "=", "Last Month Payment") : "";
    $switch_remember['payment_last'] = $_POST['payment_last'][0];
    $preFilters_str[22] = isset($_POST['payment_sec'][0]) ? array($_POST['payment_sec'], "CLASS.PAYMENT_SEC", "=", "Security Deposit") : "";
    $switch_remember['payment_sec'] = $_POST['payment_sec'][0];
    $preFilters_str[23] = isset($_POST['key_deposit'][0]) ? array($_POST['key_deposit'], "CLASS.KEY_DEPOSIT", "=", "Key Deposit") : "";
    $switch_remember['key_deposit'] = $_POST['key_deposit'][0];
    $preFilters_str[24] = isset($_POST['clean_deposit'][0]) ? array($_POST['clean_deposit'], "CLASS.CLEAN_DEPOSIT", "=", "Clean Deposit") : "";
    $switch_remember['clean_deposit'] = $_POST['clean_deposit'][0];
    $preFilters_str[25] = isset($_POST['fee_comments'][0]) ? array($_POST['fee_comments'], "CLASS.FEE_COMMENTS", "=", "Fee Comments") : "";
    $switch_remember['fee_comments'] = $_POST['fee_comments'][0];
    $preFilters_str[26] = isset($_POST['parking_num'][0]) ? array($_POST['parking_num'], "CLASS.PARKING_NUM", "=", "Number of Parking Spaces") : "";
    ...
    $preFilters_str[28] = isset($_POST['parking_cost'][0]) ? array($_POST['parking_cost'], "CLASS.PARKING_COST", "=", "Cost per Parking Space") : "";
    $switch_remember['parking_cost'] = $_POST['parking_cost'][0];
    $preFilters_str[29] = array($_POST['features_deleaded'], "CLASS.FEATURES_DELEADED", "=", "Features: Lead Safe");
    ...
    $preFilters_str[81] = isset($_POST['tenant_name'][0]) ? array($_POST['tenant_name'], "CLASS.TENANT_NAME", "=", "Tenant Name") : "";
    $switch_remember['tenant_name'] = $_POST['tenant_name'][0];
    $preFilters_str[82] = isset($_POST['tenant_phone'][0]) ? array($_POST['tenant_phone'], "CLASS.TENANT_PHONE", "=", "Tenant Phone") : "";
    ...
    $preFilters_str[85] = isset($_POST['alarm'][0]) ? array($_POST['alarm'], "CLASS.ALARM", "=", "Alarm Code") : "";
    ...
    $preFilters_str[91] = isset($_POST['street_num'][0]) ? array($_POST['street_num'], "CLASS.STREET_NUM", "=", "Street Number") : "";
    ...
    $preFilters_str[108] = isset($_POST['zip'][0]) ? array($_POST['zip'], "CLASS.ZIP", "=", "Zip Code") : "";
    ...
    // The hardcoded true/false is a temporary fix. The code that was here previously had a syntax error and was missing any logic
    // to indicate the intended behavior, replace if the proper behavior is known.
    $preFilters_str[117] = ($_POST['pic'][0] ? false : true) ? array($_POST['pic'], "PIC", ">", "Has Pics") : "";

    $preFilters_str[118] = array($_POST['not_deleaded'], "CLASS.NOT_DELEADED", "=", "Not Lead Safe");

    $preFilters_str[119] = isset($_POST['status_pending'][0]) ? array($_POST['status_pending'], "STATUS_PENDING", "=", "Pending") : "";
    ...
    $preFilters_str[121] = isset($_POST['street_num_begin'][0]) ? array($_POST['street_num_begin'], "CLASS.STREET_NUM", ">=", "Street # Begin") : "";
    $switch_remember['street_num_begin'] = $_POST['street_num_begin'];

    $preFilters_str[122] = isset($_POST['street_num_end'][0]) ? array($_POST['street_num_end'], "CLASS.STREET_NUM", "<=", "Street # End") : "";
    ...
    $preFilters_str[125] = isset($_POST['keyword'][0]) ? array($_POST['keyword'], "CLASS.BODY", " LIKE " . $_POST['keyword'], "Keyword to Match") : "";
    $switch_remember['keyword'] = $_POST['keyword'];

    $preFilters_str[126] = array($_POST['amenities_bikeroom'], "CLASS.AMENITIES_BIKEROOM", "=", "Amenities: Bike Storage");
    $switch_remember['amenities_bikeroom'] = $_POST['amenities_bikeroom'];

    $preFilters_str[127] = array($_POST['features_cabletv'], "CLASS.FEATURES_CABLETV", "=", "Features: Cable TV");
    $switch_remember['features_cabletv'] = $_POST['features_cabletv'];

    $preFilters_str[128] = array($_POST['amenities_playground'], "CLASS.AMENITIES_PLAYGROUND", "=", "Amenities: Playground");
    $switch_remember['amenities_playground'] = $_POST['amenities_playground'];

    $preFilters_str[128] = array($_POST['amenities_basketball'], "CLASS.AMENITIES_BASKETBALL", "=", "Amenities: Basketball Court");
    $switch_remember['amenities_basketball'] = $_POST['amenities_basketball'];

    $preFilters_str[129] = array($_POST['features_stainless'], "CLASS.FEATURES_STAINLESS", "=", "Features: Stainless Steel");
    $switch_remember['features_stainless'] = $_POST['features_stainless'];

    $preFilters_str[130] = array($_POST['features_granitec'], "CLASS.FEATURES_GRANITEC", "=", "Features: Granite Counters");
    $switch_remember['features_granitec'] = $_POST['features_granitec'];

    $preFilters_str[131] = array($_POST['features_baywindows'], "CLASS.FEATURES_BAYWINDOWS", "=", "Features: Bay Windows");
    $switch_remember['features_baywindows'] = $_POST['features_baywindows'];

    $preFilters_str[132] = array($_POST['amenities_bbq'], "CLASS.AMENITIES_BBQ", "=", "Amenities: BBQ");
    $switch_remember['amenities_bbq'] = $_POST['amenities_bbq'];
    $preFilters_str[133] = array($_POST['amenities_zipcar'], "CLASS.AMENITIES_ZIPCAR", "=", "Amenities: ZipCar");
    $switch_remember['amenities_zipcar'] = $_POST['amenities_zipcar'];

    $preFilters_str[134] = array($_POST['features_intercom'], "CLASS.FEATURES_INTERCOM", "=", "Features: Intercom");
    $switch_remember['features_intercom'] = $_POST['features_intercom'];
    $preFilters_str[135] = array($_POST['features_ceilingfan'], "CLASS.FEATURES_CEILINGFAN", "=", "Features: Ceiling Fan");
    $switch_remember['features_ceilingfan'] = $_POST['features_ceilingfan'];

    $preFilters_str[136] = ($_POST['nopic'][0] = 0) ? array($_POST['nopic'], "PIC", "<", "Has No Pics") : "";
    ...
    if (isset($_POST['sourcemls'])) {
        ...
        } elseif ($_POST['sourcemls'] == "RJ") {
            $defaultmls = "5";
            $switch_remember['sourcemls'] = 'RJ';
        } elseif ($_POST['sourcemls'] == "YGL") {
            $defaultmls = "6";
            $switch_remember['sourcemls'] = 'YGL';
...
if (isset($clientFilterO->CLID)) {
    ...
    if ($clientFilterO->LEADSAFE) {
        $preFilters_str[29] = array(array("1"), "CLASS.FEATURES_DELEADED", "=", "Features: Lead Safe");
        $switch_remember['features_deleaded'] = "1";
    }
    if (!empty($preFilters_str[5])) {
        echo $preFilters_str[5];
    } else {
        echo "index error <test>";
    }
}
	...
	<td bgcolor="<?php echo $pagebgcolor; ?>" align="center">
		<table>
			...
			<td height="30"  bgcolor="<?php echo $pagebgcolor; ?>">
                ...
                <select class="form-control" name="type[]" multiple size="3" STYLE="Background-Color : #FFFFFF">
				<?php while ($rowTypes = mysqli_fetch_object($quTypes)) {
        ?>
					<option value="<?php echo $rowTypes->TYPE; ?>"
					<?php if (isset($switch_remember['type']) and is_array($switch_remember['type'])) {
            if (in_array($rowTypes->TYPE, $switch_remember['type'])) {
                echo " selected ";
            }
        } elseif (isset($switch_remember['type']) and $rowTypes->TYPE == $switch_remember['type']) {
            echo " selected ";
        } elseif ($rowTypes->TYPE == $rowGetUser->AGENT_TYPE) {
            echo " selected ";
			...
			<td height="30"  bgcolor="<?php echo $pagebgcolor; ?>">
<?php if ($rowGetUser->PREF_SHOW_LL_SEARCH == "1") {echo "&nbsp;";} else {
        ?>
                ...
                <select class="form-control" name="landlord[]" multiple size="3" STYLE="Background-Color : #FFFFFF">
        				<?php while ($rowGetLandlord = mysqli_fetch_object($quLandlord)) {
            ?>
        					<option value="<?php echo $rowGetLandlord->LID; ?>"
        					<?php if (isset($switch_remember['landlord']) and is_array($switch_remember['landlord'])) {
...
<?php }?>
			...
			<td height="30" bgcolor="<?php echo $pagebgcolor; ?>">
                ...
                <select class="form-control" name="loc[]" multiple size="3" STYLE="Background-Color : #FFFFFF">
        	<?php while ($rowFavLocs = mysqli_fetch_object($quFavLocs)) {
        ?>
        	<option value="<?php echo $rowFavLocs->LOCID; ?>" <?php if (isset($rowGetAd->LOC) and $rowGetAd->LOC == $rowFavLocs->LOCID) {
            echo " selected";
            $locSeled = true;}?>><?php echo $rowFavLocs->LOCNAME; ?></option>
        	<?php }?>
        	<option value="0">--------------------</option>
        <option value="34,35,36,37,38,39,40,41,42,398,86,43,44,45,46,47,381,48,49,404,389,50,51,177,52,53,400,54,55,388,56,395,57,58,379,386,59,60,61,62,63,382,387,64,378,65,66,67">BOSTON - All areas</option>
        <option value="78,461,462,463,464,465,466,467,468,469,470,471,472,473,474,475,476,477,478,479,480">BROOKLINE - All areas</option>
        <option value="81,411,412,413,414,415,416,417,442,418,441,419,420,421,443,422,423,445,424,425,426">CAMBRIDGE - All areas</option>
        <option value="244,448,449,452,453,450,451,455,456,457,458,454,459,460">NEWTON - All areas</option>
        <option value="310,427,428,429,430,440,439,444,431,432,433,434,435,436,437,438">SOMERVILLE - All areas</option>
        	<option value="0">--------------------</option>

        <?php while ($rowLocs = mysqli_fetch_object($quLocs)) {
        ?>
        				<option value="<?php echo $rowLocs->LOCID; ?>"
        			<?php if (isset($switch_remember['loc']) and is_array($switch_remember['loc'])) {
            if (in_array($rowLocs->LOCID, $switch_remember['loc'])) {
                echo " selected ";
            }
        } elseif (isset($switch_remember['loc'])) {
            echo " selected ";
        }?>>
	...
	<td bgcolor="<?php echo $pagebgcolor; ?>" align="center">
		<table BORDER=0>
			...
			<td height="30" width="80" bgcolor="<?php echo $pagebgcolor; ?>">
                <div class="controltext">
                   <!--  <NOBR>Price Min:</NOBR>
                    <BR>
                    $<input type="text" name="priceStart[]" size="4" value="<?php echo $switch_remember['priceStart']; ?>">
                    -->
                    ...
                    <!-- <BR> -->
                      ...
                      <input class="form-control" type="text" name="priceStart[]" value = "<?php echo $switch_remember['priceStart']; ?>" size="4" >
                    </div>
                </div>
            </td>
			<td height="30" width="1" bgcolor="<?php echo $pagebgcolor; ?>">&nbsp;</td>
			<td height="30" width="80" bgcolor="<?php echo $pagebgcolor; ?>">
                <div class="controltext">
                   <!--  <NOBR>Price Max:</NOBR>
                    <BR>
                    $<input type="text" name="priceEnd[]" size="4" value="<?php echo $switch_remember['priceEnd']; ?>">
 -->
                    <div class="controltext input-group mb-3"><nobr>Price Max:</nobr>
                    <div class="input-group-prepend">
                        <span class="input-group-text" id="basic-addon1">$</span>
                      </div>
                      <input class="form-control" type="text" name="priceEnd[]" value="<?php echo $switch_remember['priceEnd']; ?>" size="4" >
                    </div>

                </div>
            </td>
			<td height="30" width="30" bgcolor="<?php echo $pagebgcolor; ?>">&nbsp;</td>

			<td height="30" bgcolor="<?php echo $pagebgcolor; ?>" align="left">
                <div class="controltext" align="left">
                    <!-- <NOBR>
                        Available:&nbsp;&nbsp;&nbsp;&nbsp;
                        <input type="checkbox" name="status_active[]" value="1" <?php if (isset($switch_remember['status_active'])) {echo " checked ";}?> >
                    </NOBR> -->
                    <div class="checkbox ">
                            <label>
                                <input type="checkbox" name="status_active[]" value="1"
                                <?php if (isset($switch_remember['status_active'])) {echo " checked ";}?>
                                >
                   ...
                   <!--  <NOBR>
                        Not Available:
                        <input type="checkbox" name="status_not_active[]" value="1" <?php if (isset($switch_remember['status_not_active'])) {echo " checked ";}?> >
                    </NOBR> -->
                    <div class="checkbox ">
                            <label>
                                <input type="checkbox" name="status_not_active[]" value="1"
                                <?php if (isset($switch_remember['status_not_active'])) {echo " checked ";}?>
                                >
			...
			<td height="30"   bgcolor="<?php echo $pagebgcolor; ?>" align="left">
                <div class="controltext"  align="left">
                   <!--  <NOBR>
                        Advertising:
                        <input type="checkbox" name="status[]" <?php if (isset($switch_remember['status']) and $switch_remember['status'] == 'ACT') {echo checked;}?> value="ACT">
                    </NOBR> -->
                     <div class="checkbox ">
                            <label>
                                <input type="checkbox" name="status[]" value="ACT"
                                <?php if (isset($switch_remember['status']) and $switch_remember['status'] == 'ACT') {echo checked;}?>
                                >
                    ...
                    <!-- <BR>
                    <NOBR>Vacant:&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                        <input type="checkbox" name="status_vacant[]" value="1" <?php if (isset($switch_remember['status_vacant'])) {echo " checked ";}?> >
                    </NOBR> -->
                     <div class="checkbox ">
                            <label>
                                <input type="checkbox" name="status_vacant[]" value="1"
                                <?php if (isset($switch_remember['status_vacant'])) {echo " checked ";}?>
                                >
			...
			<td height="30"  bgcolor="<?php echo $pagebgcolor; ?>" >
                <div class="controltext" align="left">
                        <!-- <NOBR>Furnished:&nbsp;&nbsp;&nbsp;&nbsp;
                            <input type="checkbox" name="features_furnished[]" value="1" <?php if (isset($switch_remember['features_furnished'])) {echo "checked";}?> >
                        </NOBR>
                        <BR> -->

                        <div class="checkbox ">
                            <label>
                                <input type="checkbox" name="features_furnished[]" value="1"
                                <?php if (isset($switch_remember['features_furnished'])) {echo "checked";}?>
                                >
                    ...
                    <!-- <NOBR>
                        Not Furnished:
                        <input type="checkbox" name="features_not_furnished[]" value="1" <?php if (isset($switch_remember['features_not_furnished'])) {echo " checked ";}?> >
                    </NOBR> -->
                    <div class="checkbox ">
                            <label>
                                <input type="checkbox" name="features_not_furnished[]" value="1"
                                <?php if (isset($switch_remember['features_not_furnished'])) {echo " checked ";}?>
                                >
	...
	<td bgcolor="<?php echo $pagebgcolor; ?>" align="center">
		...
		<table>
			...
			<td height="30" width="20" bgcolor="<?php echo $pagebgcolor; ?>"><div class="controltext">Month</div> <select name="bbbmonth" STYLE="Background-Color : #FFFFFF">
				<option value="--" <?php if (isset($switch_remember['bbbmonth']) and $switch_remember['bbbmonth'] == "--") {echo " selected ";}?>>--</option>
				<option value="01" <?php if (isset($switch_remember['bbbmonth']) and $switch_remember['bbbmonth'] == "01") {echo " selected ";}?> >Jan</option>
				<option value="02" <?php if (isset($switch_remember['bbbmonth']) and $switch_remember['bbbmonth'] == "02") {echo " selected ";}?> >Feb</option>
				<option value="03" <?php if (isset($switch_remember['bbbmonth']) and $switch_remember['bbbmonth'] == "03") {echo " selected ";}?> >Mar</option>
				<option value="04" <?php if (isset($switch_remember['bbbmonth']) and $switch_remember['bbbmonth'] == "04") {echo " selected ";}?> >Apr</option>
				<option value="05" <?php if (isset($switch_remember['bbbmonth']) and $switch_remember['bbbmonth'] == "05") {echo " selected ";}?> >May</option>
				<option value="06" <?php if (isset($switch_remember['bbbmonth']) and $switch_remember['bbbmonth'] == "06") {echo " selected ";}?> >Jun</option>
				<option value="07" <?php if (isset($switch_remember['bbbmonth']) and $switch_remember['bbbmonth'] == "07") {echo " selected ";}?> >Jul</option>
				<option value="08" <?php if (isset($switch_remember['bbbmonth']) and $switch_remember['bbbmonth'] == "08") {echo " selected ";}?> >Aug</option>
				<option value="09" <?php if (isset($switch_remember['bbbmonth']) and $switch_remember['bbbmonth'] == "09") {echo " selected ";}?> >Sep</option>
				<option value="10" <?php if (isset($switch_remember['bbbmonth']) and $switch_remember['bbbmonth'] == "10") {echo " selected ";}?> >Oct</option>
				<option value="11" <?php if (isset($switch_remember['bbbmonth']) and $switch_remember['bbbmonth'] == "11") {echo " selected ";}?> >Nov</option>
				<option value="12" <?php if (isset($switch_remember['bbbmonth']) and $switch_remember['bbbmonth'] == "12") {echo " selected ";}?> >Dec</option>
				</select></td>
			<td height="30" width="1" bgcolor="<?php echo $pagebgcolor; ?>">&nbsp;</td>
			<td height="30" width="20" bgcolor="<?php echo $pagebgcolor; ?>"><div class="controltext">Day</div><select name="bbbday" STYLE="Background-Color : #FFFFFF">
				<option value="--" <?php if (isset($switch_remember['bbbday']) and $switch_remember['bbbday'] == "--") {echo " selected ";}?>>--</option>
				<?php for ($i = 1; $i <= 31; $i++) {
        if ($i <= 9) {
            $j = "0" . $i;
        } else {
        ...
        ?>
				<option value="<?php echo $j; ?>" <?php if (isset($switch_remember['bbbday']) and $switch_remember['bbbday'] == $j) {echo " selected ";}?>><?php echo $j; ?></option>
				<?php }?>
			</select></td>
			<td height="30" width="1" bgcolor="<?php echo $pagebgcolor; ?>">&nbsp;</td>
			<td height="30" width="20" bgcolor="<?php echo $pagebgcolor; ?>"><div class="controltext">Year</div><select name="bbbyear" STYLE="Background-Color : #FFFFFF">
				<option value="--" <?php if (isset($switch_remember['bbbyear']) and $switch_remember['bbbyear'] == "--") {echo " selected ";}?>>--</option>
				<?php
$thisYear = date("Y") - 3;
    for ($i = 0; $i <= 7; $i++) {?>
				<option value="<?php echo ($thisYear + $i); ?>" <?php if (isset($switch_remember['bbbyear']) and $switch_remember['bbbyear'] == ($thisYear + $i)) {echo " selected ";}?>><?php echo ($thisYear + $i); ?></option>
				<?php }?>
				</select></td>
			<td height="30" width="1" bgcolor="<?php echo $pagebgcolor; ?>">&nbsp;</td>
			<td height="30" width="1" bgcolor="<?php echo $pagebgcolor; ?>">&nbsp;</td>
			<td height="30" width="20" bgcolor="<?php echo $pagebgcolor; ?>"><div class="controltext">Month</div><select name="bbemonth" STYLE="Background-Color : #FFFFFF">
				<option value="--" <?php if (isset($switch_remember['bbemonth']) and $switch_remember['bbemonth'] == "--") {echo " selected";}?>>--</option>
				<option value="01" <?php if (isset($switch_remember['bbemonth']) and $switch_remember['bbemonth'] == "01") {echo " selected";}?>>Jan</option>
				<option value="02" <?php if (isset($switch_remember['bbemonth']) and $switch_remember['bbemonth'] == "02") {echo " selected";}?>>Feb</option>
				<option value="03" <?php if (isset($switch_remember['bbemonth']) and $switch_remember['bbemonth'] == "03") {echo " selected";}?>>Mar</option>
				<option value="04" <?php if (isset($switch_remember['bbemonth']) and $switch_remember['bbemonth'] == "04") {echo " selected";}?>>Apr</option>
				<option value="05" <?php if (isset($switch_remember['bbemonth']) and $switch_remember['bbemonth'] == "05") {echo " selected";}?>>May</option>
				<option value="06" <?php if (isset($switch_remember['bbemonth']) and $switch_remember['bbemonth'] == "06") {echo " selected";}?>>Jun</option>
				<option value="07" <?php if (isset($switch_remember['bbemonth']) and $switch_remember['bbemonth'] == "07") {echo " selected";}?>>Jul</option>
				<option value="08" <?php if (isset($switch_remember['bbemonth']) and $switch_remember['bbemonth'] == "08") {echo " selected";}?>>Aug</option>
				<option value="09" <?php if (isset($switch_remember['bbemonth']) and $switch_remember['bbemonth'] == "09") {echo " selected";}?>>Sep</option>
				<option value="10" <?php if (isset($switch_remember['bbemonth']) and $switch_remember['bbemonth'] == "10") {echo " selected";}?>>Oct</option>
				<option value="11" <?php if (isset($switch_remember['bbemonth']) and $switch_remember['bbemonth'] == "11") {echo " selected";}?>>Nov</option>
				<option value="12" <?php if (isset($switch_remember['bbemonth']) and $switch_remember['bbemonth'] == "12") {echo " selected";}?>>Dec</option>
				</select></td>
			<td height="30" width="1" bgcolor="<?php echo $pagebgcolor; ?>">&nbsp;</td>
			<td height="30" width="20" bgcolor="<?php echo $pagebgcolor; ?>"><div class="controltext">Day</div><select name="bbeday" STYLE="Background-Color : #FFFFFF">
				<option value="--" <?php if (isset($switch_remember['bbeday']) and $switch_remember['bbeday'] == "--") {echo " selected ";}?>>--</option>
				<?php for ($i = 1; $i <= 31; $i++) {
        if ($i <= 9) {
            $j = "0" . $i;
        } else {
        ...
        ?>
				<option value="<?php echo $j; ?>" <?php if (isset($switch_remember['bbeday']) and $switch_remember['bbeday'] == $j) {echo " selected ";}?>><?php echo $j; ?></option>
				<?php }?>
				</select></td>
			<td height="30" width="1" bgcolor="<?php echo $pagebgcolor; ?>">&nbsp;</td>
			<td height="30" width="20" bgcolor="<?php echo $pagebgcolor; ?>"><div class="controltext">Year</div><select name="bbeyear" STYLE="Background-Color : #FFFFFF">
				<option value="--" <?php if (isset($switch_remember['bbeyear']) and $switch_remember['bbeyear'] == "--") {echo " selected ";}?>>--</option>
				<?php
$thisYear = date("Y") - 3;
    for ($i = 0; $i <= 7; $i++) {?>
				<option value="<?php echo ($thisYear + $i); ?>" <?php if (isset($switch_remember['bbeyear']) and $switch_remember['bbeyear'] == ($thisYear + $i)) {echo " selected ";}?> ><?php echo ($thisYear + $i); ?></option>
				<?php }?>
				</select></td>
			</tr>
			</table></td>
	...
	<td bgcolor="<?php echo $pagebgcolor; ?>">
			...
			<td height="30" bgcolor="<?php echo $pagebgcolor; ?>"><div class="controltext">Bedrooms:</div><select class="form-control" name="rooms[]" multiple size="3" STYLE="Background-Color : #FFFFFF">
			<?php foreach ($DEFINED_VALUE_SETS['ROOMS'] as $roomKey => $roomVal) {
        ?>
				<option value="<?php echo $roomKey; ?>"
				<?php if (isset($switch_remember['rooms']) and is_array($switch_remember['rooms'])) {
            if (in_array($roomKey, $switch_remember['rooms'])) {
                echo " selected ";
            }
        } elseif (isset($switch_remember['rooms']) and $switch_remember['rooms'] == $roomKey) {
            echo " selected ";
			...
			<td height="30" width="1" bgcolor="<?php echo $pagebgcolor; ?>">&nbsp;</td>
			<td height="30" bgcolor="<?php echo $pagebgcolor; ?>"><div class="controltext"><NOBR>Bath(s):</NOBR></div><select name="bath[]" class="form-control" multiple size="3" STYLE="Background-Color : #FFFFFF">
				<?php foreach ($DEFINED_VALUE_SETS['BATH'] as $bathKey => $bathVal) {
        ?>
				<option value="<?php echo $bathKey; ?>"
				<?php if (isset($switch_remember['bath']) and is_array($switch_remember['bath'])) {
            if (in_array($bathKey, $switch_remember['bath'])) {
                echo " selected ";
            }
        } elseif (isset($switch_remember['bath']) and $switch_remember['bath'] == $bathKey) {
            echo " selected ";
        }?>>
			...
			<td height="30" width="1" bgcolor="<?php echo $pagebgcolor; ?>">&nbsp;</td>
			<td height="30"  bgcolor="<?php echo $pagebgcolor; ?>"><div class="controltext">Pets:</div><select class="form-control" name="pets[]" multiple size="3" STYLE="Background-Color : #FFFFFF">
				<?php foreach ($DEFINED_VALUE_SETS['PETSA'] as $petKey => $petVal) {
        ?>
					<option value="<?php echo $petKey; ?>"
					<?php if (isset($switch_remember['pets']) and is_array($switch_remember['pets'])) {
            if (in_array($petKey, $switch_remember['pets'])) {
                echo " selected ";
            }
        } elseif (isset($switch_remember['pets']) and $switch_remember['pets'] == $petKey) {
            echo " selected ";
        }?>>
...
Ad Body Keyword: <input type="text" size="10" name="keyword" value="<?php echo $switch_remember['keyword']; ?>" STYLE="Background-Color : #FFFFFF; height :20px; font-size : 8pt;">



Source:
<SELECT NAME="sourcemls" STYLE="Background-Color : #FFFFFF">
<OPTION VALUE="<?php echo $grid; ?>" <?php if ($switch_remember['sourcemls'] == "$grid") {
        echo "selected";
    } elseif ($rowGetUser->SOURCEPREF == "$grid") {echo "selected";}?>>Our Office Only
<OPTION VALUE="1075" <?php if ($switch_remember['sourcemls'] == "1075" or $_SESSION["sourcepref"] == "1075") {echo "selected";}?>>MLS + OFFICE
<OPTION VALUE="1075A" <?php if ($switch_remember['sourcemls'] == "1075A" or $_SESSION["sourcepref"] == "1075A") {echo "selected";}?>>MLS Only
<OPTION VALUE="BA" <?php if ($switch_remember['sourcemls'] == "BA" or $_SESSION["sourcepref"] == "BA") {echo "selected";}?>>BA Network Only
<OPTION VALUE="BAA" <?php if ($switch_remember['sourcemls'] == "BA" or $_SESSION["sourcepref"] == "BA") {echo "selected";}?>>BA Network + OFFICE
<OPTION VALUE="YGL" <?php if ($switch_remember['sourcemls'] == "YGL" or $_SESSION["sourcepref"] == "YGL") {echo "selected";}?>>YGL Only
<OPTION VALUE="YGLA" <?php if ($switch_remember['sourcemls'] == "YGLAS" or $_SESSION["sourcepref"] == "YGLA") {echo "selected";}?>>YGL + OFFICE
<OPTION VALUE="YGLC" <?php if ($switch_remember['sourcemls'] == "YGL" or $_SESSION["sourcepref"] == "YGLB") {echo "selected";}?>>YGL+MLS+OFFICE
<OPTION VALUE="ALL" <?php if ($switch_remember['sourcemls'] == "ALL" or $_SESSION["sourcepref"] == "ALL") {echo "selected";}?>>ALL Sources
</SELECT>
</NOBR>
</div>
...
<?php } elseif ($listing_filter_display == "small") {
	...
	<tr>
	<td valign="top" height="30" width="1" bgcolor="#000000"><img border="0" hspace="0" vspace="0" width="1" height="100%" src="../assets/images/blk_spacer.gif"></td>
	<td class = "p-2" bgcolor="<?php echo $pagebgcolor; ?>" align="">
		<table width = "100%">
			<tr>

			<td height="30"  bgcolor="<?php echo $pagebgcolor; ?>">
				<div class="controltext "><NOBR>Listing Type(s):</NOBR></div>
				<select class = "form-control" name="type[]" multiple size="3" STYLE="Background-Color : #FFFFFF; font-size : 8pt;">
				<?php while ($rowTypes = mysqli_fetch_object($quTypes)) {
        ?>
					<option value="<?php echo $rowTypes->TYPE; ?>"
					<?php if (isset($switch_remember['type']) and is_array($switch_remember['type'])) {
        ...
        } elseif (isset($rowGetUser->AGENT_TYPE) and $rowTypes->TYPE == $rowGetUser->AGENT_TYPE) {
            echo " selected ";
			...
			<td height="30" bgcolor="<?php echo $pagebgcolor; ?>">
<?php if (isset($rowGetUser) and $rowGetUser->PREF_SHOW_LL_SEARCH == "1") {echo "&nbsp;";} else {
        ?>

<div class="controltext"><NOBR>Landlord/Owner(s):</NOBR></DIV><select class = "form-control" name="landlord[]" multiple size="3" STYLE="Background-Color : #FFFFFF; font-size : 8pt;">
				<?php while ($rowGetLandlord = mysqli_fetch_object($quLandlord)) {
            ?>
					<option value="<?php echo $rowGetLandlord->LID; ?>"
					<?php if (isset($switch_remember['landlord']) and is_array($switch_remember['landlord'])) {
...
<?php }?>
			...
			<td height="30"  bgcolor="<?php echo $pagebgcolor; ?>"><div class="controltext">Location(s):</div>
				<select class = "form-control" name="loc[]" multiple size="3" STYLE="Background-Color : #FFFFFF; font-size : 8pt;">
				<?php while ($rowFavLocs = mysqli_fetch_object($quFavLocs)) {
        ?>
...
<option value="310,427,428,429,430,440,439,444,431,432,433,434,435,436,437,438">SOMERVILLE - All areas</option>
	<option value="0">--------------------</option>
        ...
        ?>
			...
			<td height="30" bgcolor="<?php echo $pagebgcolor; ?>"><div class="controltext">Bedrooms:</div>
				<select class = "form-control" name="rooms[]" multiple size="3" STYLE="Background-Color : #FFFFFF; font-size : 8pt;">
			<?php foreach ($DEFINED_VALUE_SETS['ROOMS'] as $roomKey => $roomVal) {
        ?>
				<option value="<?php echo $roomKey; ?>"
				<?php if (isset($switch_remember['rooms']) and is_array($switch_remember['rooms'])) {
			...
			<td height="30" bgcolor="<?php echo $pagebgcolor; ?>"><div class="controltext"><NOBR>Bath(s):</NOBR></div>
				<select class = "form-control" name="bath[]" multiple size="3" STYLE="Background-Color : #FFFFFF; font-size : 8pt;">
				<?php foreach ($DEFINED_VALUE_SETS['BATH'] as $bathKey => $bathVal) {
        ?>
				<option value="<?php echo $bathKey; ?>"
				<?php if (isset($switch_remember['bath']) and is_array($switch_remember['bath'])) {
   ...
   <td height="30" bgcolor="<?php echo $pagebgcolor; ?>"><div class="controltext">Pets:</div>
   	<select class = "form-control" name="pets[]" multiple size="3" STYLE="Background-Color : #FFFFFF; font-size : 8pt;">
				<?php foreach ($DEFINED_VALUE_SETS['PETSA'] as $petKey => $petVal) {
        ?>
					<option value="<?php echo $petKey; ?>"
					<?php if (isset($switch_remember['pets']) and is_array($switch_remember['pets'])) {
		...
		<table BORDER=0>
			...
			<td height="30" width="80" bgcolor="<?php echo $pagebgcolor; ?>">
				<div class="controltext input-group mb-3">
					...
					<!-- <BR> -->
					  ...
					  <input class = "form-control" type="text" name="priceStart[]" size="4" value="<?php echo isset($switch_remember['priceStart']) ? $switch_remember['priceStart'] : ''; ?>">
					</div>
					<!-- $<input class = "form-control" type="text" name="priceStart[]" size="4" value="<?php echo isset($switch_remember['priceStart']) ? $switch_remember['priceStart'] : ''; ?>"> -->
				</div>
			</td>
			...
			<td height="30" width="80" bgcolor="<?php echo $pagebgcolor; ?>">
				<div class="controltext input-group mb-3"><NOBR>Price Max:</NOBR>
					<div class="input-group-prepend">
					  ...
					  <input class="form-control" type="text" name="priceEnd[]" size="4" value="<?php echo isset($switch_remember['priceEnd']) ? $switch_remember['priceEnd'] : ''; ?>">
					</div>
					<!-- <BR> -->
					<!-- $<input type="text" name="priceEnd[]" size="4" value="<?php echo isset($switch_remember['priceEnd']) ? $switch_remember['priceEnd'] : ''; ?>"> -->
				</div>
			</td>
			<td height="30" width="1" bgcolor="<?php echo $pagebgcolor; ?>">&nbsp;</td>

			<td height="30" bgcolor="<?php echo $pagebgcolor; ?>" align="">
				<div class="controltext med-font" align="">
					<!-- <NOBR>Available:&nbsp;&nbsp;&nbsp;&nbsp; -->
						<div class="checkbox ">
						    <label>
								<input type="checkbox" name="status_active[]" value="1" <?php if (isset($switch_remember['status_active'])) {echo " checked ";}?> >
					...
					<!-- <BR> -->
					<!-- <NOBR>Has Pics: &nbsp;&nbsp;&nbsp;&nbsp;
						<input type="checkbox" name="pic[]" value="1" <?php if (isset($switch_remember['pic'])) {echo " checked ";}?> >
					</NOBR> -->
						<div class="checkbox">
						    <label>
								<input type="checkbox" name="pic[]" value="1" <?php if (isset($switch_remember['pic'])) {echo " checked ";}?> >
			...
			<td height="30"  bgcolor="<?php echo $pagebgcolor; ?>" align="">
				<div class="controltext med-font">
					...
					<NOBR>Priority:&nbsp;&nbsp;&nbsp;
						<input type="checkbox" name="priority[]" value="1" <?php if (isset($switch_remember['priority'])) {echo " checked ";}?> >
					</NOBR> -->
			...
			<td height="30"  bgcolor="<?php echo $pagebgcolor; ?>">
				<div class="controltext med-font" align="">
						...
						<!-- <BR>
						<NOBR>Lead Safe:&nbsp;&nbsp;&nbsp;&nbsp;
							<input type="checkbox" name="features_deleaded[]" value="1" <?php if (isset($switch_remember['features_deleaded'])) {echo "checked";}?> >
						</NOBR>
						<BR> -->
			...
			<TD VALIGN="MIDDLE">
				<div class="controltext med-font p-2">
					Parking:
					<div class="controltext med-font">

 					<select class = "form-control" name="parking_num[]" STYLE="Background-Color : #FFFFFF; " size="2" multiple>
						<OPTION VALUE=""> -- </OPTION>

						<?php foreach ($DEFINED_VALUE_SETS['PARKING_NUM'] as $pknKey => $pknVal) {
        ?>
						<option value="<?php echo $pknKey; ?>"
						<?php if (isset($switch_remember['parking_num']) and is_array($switch_remember['parking_num'])) {
...
<td bgcolor="<?php echo $pagebgcolor; ?>" align="">
...
$getMon = date('m');
    ...
    ?>
		...
		<table CELLPADDING="0" CELLSPACING="0" BORDER=0>
			...
			<td width="200" bgcolor="<?php echo $pagebgcolor; ?>" align="">
				<div class="controltext">
					<NOBR><B>Available begin date:</B></NOBR>
				</div>
			</td>
			<td width="1" bgcolor="<?php echo $pagebgcolor; ?>">&nbsp;</td>
			<td width="200" bgcolor="<?php echo $pagebgcolor; ?>" align="center">
				<div class="controltext">
					<NOBR><B>Available end date:</B></NOBR>
				</div>
			</td>
			</tr>
					...
					<table>
						<tr>
							<td width="20" bgcolor="<?php echo $pagebgcolor; ?>">
								<div class="controltext">Month</div>
								<select name="bbbmonth" STYLE="Background-Color : #FFFFFF; font-size : 8pt;">
									<option value="--" <?php if (isset($switch_remember['bbbmonth']) and $switch_remember['bbbmonth'] == "--") {echo " selected ";}?>>--</option>
									<option value="01" <?php if (isset($switch_remember['bbbmonth']) and $switch_remember['bbbmonth'] == "01") {echo " selected ";}?> >Jan</option>
									<option value="02" <?php if (isset($switch_remember['bbbmonth']) and $switch_remember['bbbmonth'] == "02") {echo " selected ";}?> >Feb</option>
									<option value="03" <?php if (isset($switch_remember['bbbmonth']) and $switch_remember['bbbmonth'] == "03") {echo " selected ";}?> >Mar</option>
							...
							<td width="1" bgcolor="<?php echo $pagebgcolor; ?>">&nbsp;</td>
							<td width="20" bgcolor="<?php echo $pagebgcolor; ?>">
								<div class="controltext">Day</div>
								<select name="bbbday" STYLE="Background-Color : #FFFFFF; font-size : 8pt;">
									<option value="--" <?php if (isset($switch_remember['bbbday']) and $switch_remember['bbbday'] == "--") {echo " selected ";}?>>--</option>
				<?php for ($i = 1; $i <= 31; $i++) {
        if ($i <= 9) {
            $j = "0" . $i;
        ...
        ?>
				...
				<?php }?>
							...
							<td width="1" bgcolor="<?php echo $pagebgcolor; ?>">&nbsp;</td>
							<td width="20" bgcolor="<?php echo $pagebgcolor; ?>">
								<div class="controltext">Year</div>
								<select name="bbbyear" STYLE="Background-Color : #FFFFFF; font-size : 8pt;">
									<option value="--" <?php if (isset($switch_remember['bbbyear']) and $switch_remember['bbbyear'] == "--") {echo " selected ";}?>>--</option>
				<?php
$thisYear = date("Y") - 3;
    for ($i = 0; $i <= 7; $i++) {?>
...
<NOBR><FONT SIZE="-2">Begin #</FONT></NOBR><BR><NOBR>(>=):</NOBR></TD><TD><input type="text"  TITLE="Exact Street # (>=)" size="3" name="street_num_begin" value="<?php echo isset($switch_remember['street_num_begin']) ? $switch_remember['street_num_begin'] : ''; ?>">
</TD><TD>&nbsp;

</TD><TD>
<NOBR><FONT SIZE="-2">End #:</FONT></NOBR><BR><NOBR>(<=)</NOBR></TD><TD><input type="text" size="3"  TITLE="Exact Street # (<=)" name="street_num_end" value="<?php echo isset($switch_remember['street_num_end']) ? $switch_remember['street_num_end'] : ''; ?>">
</TD></TR></TABLE>
</TD><TD>

			<NOBR>Street #:<input type="text" size="5" name="street_num" value="<?php echo isset($switch_remember['street_num']) ? $switch_remember['street_num'] : ''; ?>" STYLE="Background-Color : #FFFFFF; height :20px; font-size : 8pt;"> Street Name:<input type="text" size="15" name="street" value="<?php echo isset($switch_remember['street']) ? $switch_remember['street'] : ''; ?>" STYLE="Background-Color : #FFFFFF; height :20px; font-size : 8pt;"> Zip: <input type="text" size="5" name="zip[]" value="<?php echo isset($switch_remember['zip']) ? $switch_remember['zip'] : ''; ?>" STYLE="Background-Color : #FFFFFF; height :20px; font-size : 8pt;"> Ad Keyword: <input type="text" size="10" name="keyword" value="<?php echo isset($switch_remember['keyword']) ? $switch_remember['keyword'] : ''; ?>" STYLE="Background-Color : #FFFFFF; height :20px; font-size : 8pt;">
</TD><TD VALIGN="CENTER"><div class="controltext">
<NOBR>Source:
<SELECT NAME="sourcemls" STYLE="Background-Color : #FFFFFF">
<OPTION VALUE="<?php echo $grid; ?>" <?php if ($switch_remember['sourcemls'] == "$grid" or $_SESSION["sourcepref"] == "$grid") {echo "selected";}?>>Our Office Only
<OPTION VALUE="1075" <?php if ($switch_remember['sourcemls'] == "1075" or $_SESSION["sourcepref"] == "1075") {echo "selected";}?>>MLS + OFFICE
<OPTION VALUE="1075A" <?php if ($switch_remember['sourcemls'] == "1075A" or $_SESSION["sourcepref"] == "1075A") {echo "selected";}?>>MLS Only
<OPTION VALUE="BA" <?php if ($switch_remember['sourcemls'] == "BA" or $_SESSION["sourcepref"] == "BA") {echo "selected";}?>>BA Network Only
<OPTION VALUE="BAA" <?php if ($switch_remember['sourcemls'] == "BA" or $_SESSION["sourcepref"] == "BA") {echo "selected";}?>>BA Network + OFFICE
<OPTION VALUE="YGL" <?php if ($switch_remember['sourcemls'] == "YGL" or $_SESSION["sourcepref"] == "YGL") {echo "selected";}?>>YGL Only
<OPTION VALUE="YGLA" <?php if ($switch_remember['sourcemls'] == "YGLAS" or $_SESSION["sourcepref"] == "YGLA") {echo "selected";}?>>YGL + OFFICE
<OPTION VALUE="YGLB" <?php if ($switch_remember['sourcemls'] == "YGL" or $_SESSION["sourcepref"] == "YGLB") {echo "selected";}?>>YGL+OFFICE
<OPTION VALUE="YGLC" <?php if ($switch_remember['sourcemls'] == "YGL" or $_SESSION["sourcepref"] == "YGLB") {echo "selected";}?>>YGL+MLS+OFFICE
<OPTION VALUE="ALL" <?php if ($switch_remember['sourcemls'] == "ALL" or $_SESSION["sourcepref"] == "ALL") {echo "selected";}?>>ALL Sources
</SELECT>
</NOBR>
...
<?php } elseif ($listing_filter_display == "big") {
	...
	<td bgcolor="<?php echo $pagebgcolor; ?>">
			<table width="100%" align="center">
			<tr>
			<td height="30" width="110" bgcolor="<?php echo $pagebgcolor; ?>">

			<?php if ($rowGetUser->PREF_SHOW_LL_SEARCH == "1") {echo "&nbsp;";} else {
        ?>

			<div class="controltext"><NOBR>Landlord/Owner(s):</NOBR><BR><select name="landlord[]" multiple size="4" STYLE="Background-Color : #FFFFFF">
				<?php while ($rowGetLandlord = mysqli_fetch_object($quLandlord)) {
            ?>
					<option value="<?php echo $rowGetLandlord->LID; ?>"
					<?php if (isset($switch_remember['landlord']) and is_array($switch_remember['landlord'])) {
...
Ad Body Keyword: <input type="text" size="12" name="keyword" value="<?php echo $switch_remember['keyword']; ?>" STYLE="Background-Color : #FFFFFF; height :20px; font-size : 8pt;"></NOBR>
...
<option value="310,427,428,429,430,440,439,444,431,432,433,434,435,436,437,438">SOMERVILLE - All areas</option>
	<option value="0">--------------------</option>
        ...
        ?>
            ...
            if (isset($switch_remember['loc']) and $rowLocs->LOCID == $switch_remember['loc']) {echo " selected";}
        }?>>
				<?php echo $rowLocs->LOCNAME; ?></option>
			<?php }?>
			</select></td>
...
$thisYear = date("Y") - 3;
    for ($i = 0; $i <= 7; $i++) {?>
        ...
        ?>
				...
				<?php if (isset($switch_remember['school']) and is_array($switch_remember['school'])) {
...
<div class="controltext"><NOBR>Available:<input type="checkbox" name="status_active[]" value="1" <?php if (isset($switch_remember['status_active'])) {echo " checked ";}?> ></NOBR></DIV>
</TD><TD><div class="controltext">
<NOBR>Not Available:<input type="checkbox" name="status_not_active[]" value="1" <?php if (isset($switch_remember['status_not_active'])) {echo " checked ";}?> ></NOBR>
</DIV>
</td>
<td height="30" width="20" bgcolor="<?php echo $pagebgcolor; ?>">
<div class="controltext"><NOBR>Advertised:<input type="checkbox" name="status[]" <?php if (isset($switch_remember['status']) and $switch_remember['status'] == 'ACT') {echo checked;}?> value="ACT">
</NOBR></DIV>
</td>
<td height="30" width="20" bgcolor="<?php echo $pagebgcolor; ?>"><NOBR>
<div class="controltext">Not Advertised:<input type="checkbox" name="status[]" <?php if (isset($switch_remember['status']) and $switch_remember['status'] == 'STO') {echo checked;}?> value="STO">
</NOBR></DIV>
</TD><TD>
<div class="controltext">
<NOBR>Vacant:<input type="checkbox" name="status_vacant[]" value="1" <?php if (isset($switch_remember['status_vacant'])) {echo " checked ";}?> ></NOBR>
</DIV>
</td>


			<td height="30" width="20" bgcolor="<?php echo $pagebgcolor; ?>">
<div class="controltext"><NOBR>Furnished:<input type="checkbox" name="features_furnished[]" value="1" <?php if (isset($switch_remember['features_furnished'])) {echo "checked";}?> ></DIV></td>



			<td height="30" width="20" bgcolor="<?php echo $pagebgcolor; ?>">
<div class="controltext"><NOBR>Not Furnished:<input type="checkbox" name="features_not_furnished[]" value="1" <?php if (isset($switch_remember['features_not_furnished'])) {echo " checked ";}?> ></NOBR>
</DIV>
</td>

</td>
<td height="30" width="20" bgcolor="<?php echo $pagebgcolor; ?>"><div class="controltext"><NOBR>Has Pics:<input type="checkbox" name="pic[]" value="1" <?php if (isset($switch_remember['pic'])) {echo " checked ";}?> >
</NOBR>
</DIV></TD>
<td height="30" width="20" bgcolor="<?php echo $pagebgcolor; ?>"><div class="controltext"><NOBR>Has No Pics:<input type="checkbox" name="nopic[]" value="1" <?php if (isset($switch_remember['nopic'])) {echo " checked ";}?> >
</NOBR>
</DIV></TD>
</td>
<td height="30" width="20" bgcolor="<?php echo $pagebgcolor; ?>"><div class="controltext"><NOBR>Pending:<input type="checkbox" name="status_pending[]" value="1" <?php if (isset($switch_remember['status_pending'])) {echo " checked ";}?> >
</NOBR>
</DIV></TD>

<td height="30" width="20" bgcolor="<?php echo $pagebgcolor; ?>"><div class="controltext"><NOBR>Priority:<input type="checkbox" name="priority[]" value="1" <?php if (isset($switch_remember['priority'])) {echo " checked ";}?> >
</NOBR>
</DIV></TD>
	...
	<td bgcolor="<?php echo $pagebgcolor; ?>">
        ...
        ?>
					...
					<?php if (isset($switch_remember['nofee']) and is_array($switch_remember['nofee'])) {
...
<select name="parking_type[]"  multiple size="4" STYLE="Background-Color : #FFFFFF">
        ...
        ?>
					...
					<?php if (isset($switch_remember['parking_type']) and is_array($switch_remember['parking_type'])) {
...
Street #:<input type="text" TITLE="Partial or exact Street #" size="5" name="street_num" value="<?php echo $switch_remember['street_num']; ?>"> Street:<input type="text" size="25" name="street" value="<?php echo $switch_remember['street']; ?>">  Zip:<input type="text" size="10" name="zip[]" value="<?php echo $switch_remember['zip']; ?>" STYLE="Background-Color : #FFFFFF">
	...
	<td align="center" bgcolor="<?php echo $pagebgcolor; ?>">
        ...
        ?>
						...
						<?php if (isset($switch_remember['listing_type']) and is_array($switch_remember['listing_type'])) {
        ...
        ?>
					...
					<?php if (isset($switch_remember['lease_type']) and is_array($switch_remember['lease_type'])) {
            if (in_array($lkey, $switch_remember['lease_type'])) {
                echo " selected ";
            }
        } elseif (isset($switch_remember['lease_type']) and $switch_remember['lease_type'] == $lkey) {
            echo " selected ";
        }?>>
        ...
        ?>
					...
					<?php if (isset($switch_remember['tax_clause']) and is_array($switch_remember['tax_clause'])) {
	...
	<CENTER>
		<table>
			...
			<td valign="top">
				...
				<table border="0">
				<tr><td align="right" class="controltext">Lead Safe</td><td align="left"><input type="checkbox" name="features_deleaded[]" value="1" <?php if (isset($switch_remember['features_deleaded'])) {echo "checked";}?> ></td></tr>

				<tr><td align="right" class="controltext">Not Lead Safe</td><td align="left"><input type="checkbox" name="not_deleaded[]" value="1" <?php if (isset($switch_remember['not_deleaded'])) {echo "checked";}?> ></td></tr>


				<tr><td align="right" class="controltext">Non-smoking</td><td align="left"><input type="checkbox" name="features_non_smoking[]" value="1" <?php if (isset($switch_remember['features_non_smoking'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Alarm</td><td align="left"><input type="checkbox" name="features_alarm[]" value="1" <?php if (isset($switch_remember['features_alarm'])) {echo "checked";}?> ></td></tr>

				<tr><td align="right" class="controltext">Intercom</td><td align="left"><input type="checkbox" name="features_intercom[]" value="1" <?php if (isset($switch_remember['features_intercom'])) {echo "checked";}?> ></td></tr>

				<tr><td align="right" class="controltext">High Ceilings </td><td align="left"><input type="checkbox" name="amenities_high_ceilings[]" value="1" <?php if (isset($switch_remember['amenities_high_ceilings'])) {echo "checked";}?> ></td></tr>

				<tr><td align="right" class="controltext">Central AC </td><td align="left"><input type="checkbox" name="features_central_ac[]" value="1" <?php if (isset($switch_remember['features_central_ac'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">AC </td><td align="left"><input type="checkbox" name="features_ac[]" value="1" <?php if (isset($switch_remember['features_ac'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Dining Room </td><td align="left"><input type="checkbox" name="features_dinning_room[]" value="1" <?if (isset($switch_remember['features_dinning_room'])) { echo "checked"; } ?> ></td></tr>

				</table>
			</td>
			<td valign="top">
				<table border="0">
				<tr><td align="right" class="controltext">Hot Water</td><td align="left"><input type="checkbox" name="features_hot_water[]" value="1" <?php if (isset($switch_remember['features_hot_water'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Heat & Hot Water</td><td align="left"><input type="checkbox" name="features_ht_and_hw[]" value="1" <?php if (isset($switch_remember['features_ht_and_hw'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">All Utilities</td><td align="left"><input type="checkbox" name="features_all_utilities[]" value="1" <?php if (isset($switch_remember['features_all_utilities'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">High Speed Internet </td><td align="left"><input type="checkbox" name="features_high_speed_internet[]" value="1" <?php if (isset($switch_remember['features_high_speed_internet'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Walk-in Closet </td><td align="left"><input type="checkbox" name="features_walk_in_closet[]" value="1" <?php if (isset($switch_remember['features_walk_in_closet'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Modern Bath </td><td align="left"><input type="checkbox" name="features_modern_bath[]" value="1" <?if (isset($switch_remember['features_modern_bath'])) { echo "checked"; } ?> ></td></tr>
				<tr><td align="right" class="controltext">Whirlpool Tub</td><td align="left"><input type="checkbox" name="features_whirlpool[]" value="1" <?if (isset($switch_remember['features_whirlpool'])) { echo "checked"; } ?> ></td></tr>

				</table>
			</td>
			<td valign="top">
				<table border="0">
				<tr><td align="right" class="controltext">Working Fireplace</td><td align="left"><input type="checkbox" name="features_fireplace_working[]" value="1" <?php if (isset($switch_remember['features_fireplace_working'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Decorative Fireplace </td><td align="left"><input type="checkbox" name="features_fireplace_decor[]" value="1" <?php if (isset($switch_remember['features_fireplace_decor'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Hardwood Floors</td><td align="left"><input type="checkbox" name="features_hwfi[]" value="1" <?php if (isset($switch_remember['features_hwfi'])) {echo "checked";}?> ></td></tr>

				<tr><td align="right" class="controltext">Carpet</td><td align="left"><input type="checkbox" name="features_carpet[]" value="1" <?php if (isset($switch_remember['features_carpet'])) {echo "checked";}?> ></td></tr>


				<tr><td align="right" class="controltext">Cable TV</td><td align="left"><input type="checkbox" name="features_cabletv[]" value="1" <?php if (isset($switch_remember['features_cabletv'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Bay Windows</td><td align="left"><input type="checkbox" name="features_baywindows[]" value="1" <?php if (isset($switch_remember['features_baywindows'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Ceiling Fan</td><td align="left"><input type="checkbox" name="features_ceilingfan[]" value="1" <?php if (isset($switch_remember['features_ceilingfan'])) {echo "checked";}?> ></td></tr>

				<tr><td align="right" class="controltext">Modern Kitchen</td><td align="left"><input type="checkbox" name="features_modern_kitchen[]" value="1" <?php if (isset($switch_remember['features_modern_kitchen'])) {echo "checked";}?> ></td></tr>

				<tr><td align="right" class="controltext">Galley Kitchen</td><td align="left"><input type="checkbox" name="features_kitchen_galley[]" value="1" <?php if (isset($switch_remember['features_kitchen_galley'])) {echo "checked";}?> ></td></tr>

				<tr><td align="right" class="controltext">Kitchenette </td><td align="left"><input type="checkbox" name="features_kitchenette[]" value="1" <?php if (isset($switch_remember['features_kitchenette'])) {echo "checked";}?> ></td></tr>
				</table>
			</td>
			<td valign="top">
				<table border="0">
				<tr><td align="right" class="controltext">Eat-in-Kitchen </td><td align="left"><input type="checkbox" name="features_eat_in_kitchen[]" value="1" <?php if (isset($switch_remember['features_eat_in_kitchen'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Microwave </td><td align="left"><input type="checkbox" name="features_microwave[]" value="1" <?php if (isset($switch_remember['features_microwave'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Pantry</td><td align="left"><input type="checkbox" name="features_pantry[]" value="1" <?php if (isset($switch_remember['features_pantry'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Gas Range</td><td align="left"><input type="checkbox" name="features_gas_range[]" value="1" <?php if (isset($switch_remember['features_gas_range'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Electric Range</td><td align="left"><input type="checkbox" name="features_elec_range[]" value="1" <?php if (isset($switch_remember['features_elec_range'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Disposal</td><td align="left"><input type="checkbox" name="features_disposal[]" value="1" <?php if (isset($switch_remember['features_disposal'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Dishwasher</td><td align="left"><input type="checkbox" name="features_dishwasher[]" value="1" <?php if (isset($switch_remember['features_dishwasher'])) {echo "checked";}?> ></td></tr>


				<tr><td align="right" class="controltext">Stainless Steel</td><td align="left"><input type="checkbox" name="features_stainless[]" value="1" <?php if (isset($switch_remember['features_stainless'])) {echo "checked";}?> ></td></tr>

				<tr><td align="right" class="controltext">Granite Counter</td><td align="left"><input type="checkbox" name="features_granitec[]" value="1" <?php if (isset($switch_remember['features_granitec'])) {echo "checked";}?> ></td></tr>
			...
			<td valign="top">
				<table border="0">
				<tr><td align="right" class="controltext">Skylight </td><td align="left"><input type="checkbox" name="features_skylight[]" value="1" <?php if (isset($switch_remember['features_skylight'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Balcony </td><td align="left"><input type="checkbox" name="features_balcony[]" value="1" <?php if (isset($switch_remember['features_balcony'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Patio </td><td align="left"><input type="checkbox" name="features_patio[]" value="1" <?php if (isset($switch_remember['features_patio'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Porch </td><td align="left"><input type="checkbox" name="features_porch[]" value="1" <?php if (isset($switch_remember['features_porch'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Enclosed Porch </td><td align="left"><input type="checkbox" name="features_enclosed_porch[]" value="1" <?php if (isset($switch_remember['features_enclosed_porch'])) {echo "checked";}?> ></td></tr>

				<tr><td align="right" class="controltext">Deck </td><td align="left"><input type="checkbox" name="features_deck[]" value="1" <?if (isset($switch_remember['features_deck'])) { echo "checked"; } ?> ></td></tr>
				<tr><td align="right" class="controltext">Duplex </td><td align="left"><input type="checkbox" name="features_duplex[]" value="1" <?if (isset($switch_remember['features_duplex'])) { echo "checked"; } ?> ></td></tr>
	...
	<CENTER>
		<table>
			...
			<td valign="top">
				<table border="0">
				<tr><td align="right" class="controltext">Concierge </td><td align="left"><input type="checkbox" name="amenities_conciearge[]" value="1" <?php if (isset($switch_remember['amenities_conciearge'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Security </td><td align="left"><input type="checkbox" name="amenities_security[]" value="1" <?php if (isset($switch_remember['amenities_security'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Superintendent </td><td align="left"><input type="checkbox" name="amenities_superintendant[]" value="1" <?php if (isset($switch_remember['amenities_superintendant'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">On Site Management</td><td align="left"><input type="checkbox" name="amenities_on_site_management[]" value="1" <?php if (isset($switch_remember['amenities_on_site_management'])) {echo "checked";}?> ></td></tr>

				<tr><td align="right" class="controltext">Owner Occupied</td><td align="left"><input type="checkbox" name="amenities_owner_occupied[]" value="1" <?php if (isset($switch_remember['amenities_owner_occupied'])) {echo "checked";}?> ></td></tr>
			...
			<td valign="top">
				<table border="0">
				<tr><td align="right" class="controltext">Roof Deck </td><td align="left"><input type="checkbox" name="amenities_roof_deck[]" value="1" <?php if (isset($switch_remember['amenities_roof_deck'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Garden </td><td align="left"><input type="checkbox" name="amenities_garden[]" value="1" <?php if (isset($switch_remember['amenities_garden'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Yard </td><td align="left"><input type="checkbox" name="amenities_yard[]" value="1" <?php if (isset($switch_remember['amenities_yard'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Elevator </td><td align="left"><input type="checkbox" name="amenities_elevator[]" value="1" <?php if (isset($switch_remember['amenities_elevator'])) {echo "checked";}?> ></td></tr>

				<tr><td align="right" class="controltext">Not Owner Occupied</td><td align="left"><input type="checkbox" name="amenities_not_owner_occupied[]" value="1" <?php if (isset($switch_remember['amenities_not_owner_occupied'])) {echo "checked";}?> ></td></tr>
			...
			<td valign="top">
				<table border="0">
				<tr><td align="right" class="controltext">Health Club </td><td align="left"><input type="checkbox" name="amenities_health_club[]" value="1" <?php if (isset($switch_remember['amenities_health_club'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Pool </td><td align="left"><input type="checkbox" name="amenities_pool[]" value="1" <?php if (isset($switch_remember['amenities_pool'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Sauna </td><td align="left"><input type="checkbox" name="amenities_sauna[]" value="1" <?php if (isset($switch_remember['amenities_sauna'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Tennis </td><td align="left"><input type="checkbox" name="amenities_tennis[]" value="1" <?php if (isset($switch_remember['amenities_tennis'])) {echo "checked";}?> ></td></tr>


				<tr><td align="right" class="controltext">Playground </td><td align="left"><input type="checkbox" name="amenities_playground[]" value="1" <?php if (isset($switch_remember['amenities_playground'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Basketball </td><td align="left"><input type="checkbox" name="amenities_basketball[]" value="1" <?php if (isset($switch_remember['amenities_basketball'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">BBQ </td><td align="left"><input type="checkbox" name="amenities_bbq[]" value="1" <?php if (isset($switch_remember['amenities_bbq'])) {echo "checked";}?> ></td></tr>
...
<tr><td align="right" class="controltext">Business Center</td><td align="left"><input type="checkbox" name="amenities_businesscenter[]" value="1" <?php if (isset($switch_remember['amenities_businesscenter'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Lounge </td><td align="left"><input type="checkbox" name="amenities_lounge[]" value="1" <?php if (isset($switch_remember['amenities_lounge'])) {echo "checked";}?> ></td></tr>

				<tr><td align="right" class="controltext">Club House</td><td align="left"><input type="checkbox" name="amenities_clubhouse[]" value="1" <?php if (isset($switch_remember['amenities_clubhouse'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Storage in Attic </td><td align="left"><input type="checkbox" name="amenities_attic[]" value="1" <?php if (isset($switch_remember['amenities_attic'])) {echo "checked";}?> >  </td></tr>
				<tr><td align="right" class="controltext">Storage in Basement </td><td align="left"><input type="checkbox" name="amenities_basement[]" value="1" <?php if (isset($switch_remember['amenities_basement'])) {echo "checked";}?> >  </td></tr>
				<tr><td align="right" class="controltext">Storage in Bin </td><td align="left"><input type="checkbox" name="amenities_bin[]" value="1" <?php if (isset($switch_remember['amenities_bin'])) {echo "checked";}?> ></td></tr>

				<tr><td align="right" class="controltext">Bike Storage </td><td align="left"><input type="checkbox" name="amenities_bikeroom[]" value="1" <?php if (isset($switch_remember['amenities_bikeroom'])) {echo "checked";}?> ></td></tr>
			...
			<td valign="top">
				...
				<tr><td align="right" class="controltext">Subway</td><td align="left"><input type="checkbox" name="amenities_subway[]" value="1" <?php if (isset($switch_remember['amenities_subway'])) {echo "checked";}?> ></td></tr>
				<tr><td align="right" class="controltext">Commuter Rail</td><td align="left"><input type="checkbox" name="amenities_crail[]" value="1" <?php if (isset($switch_remember['amenities_crail'])) {echo "checked";}?> >  </td></tr>
				<tr><td align="right" class="controltext">Bus</td><td align="left"><input type="checkbox" name="amenities_bus[]" value="1" <?php if (isset($switch_remember['amenities_bus'])) {echo "checked";}?> >  </td></tr>
				<tr><td align="right" class="controltext">Shuttle bus</td><td align="left"><input type="checkbox" name="amenities_shuttle[]" value="1" <?php if (isset($switch_remember['amenities_shuttle'])) {echo "checked";}?> ></td></tr>
<tr><td align="right" class="controltext">ZipCar</td><td align="left"><input type="checkbox" name="amenities_zipcar[]" value="1" <?php if (isset($switch_remember['amenities_zipcar'])) {echo "checked";}?> ></td></tr>


				<tr><td align="right" class="controltext">Wheelchair Access</td><td align="left"><input type="checkbox" name="amenities_wheelchair[]" value="1" <?php if (isset($switch_remember['amenities_wheelchair'])) {echo "checked";}?> ></td></tr>
</table>
	...
	<td align="center" bgcolor="<?php echo $pagebgcolor; ?>">
        ...
        ?>
					...
					<?php if (isset($switch_remember['building_type']) and is_array($switch_remember['building_type'])) {
        ...
        ?>
					...
					<?php if (isset($switch_remember['building_style']) and is_array($switch_remember['building_style'])) {
        ...
        ?>
					...
					<?php if (isset($switch_remember['exterior']) and is_array($switch_remember['exterior'])) {
        ...
        ?>
					...
					<?php if (isset($switch_remember['laundry_room']) and is_array($switch_remember['laundry_room'])) {
	...
	<td align="center" bgcolor="<?php echo $pagebgcolor; ?>">
        ...
        ?>
					...
					<?php if (isset($switch_remember['key_info']) and is_array($switch_remember['key_info'])) {
        ...
        ?>
				...
				<?php if (isset($switch_remember['students']) and is_array($switch_remember['students'])) {
...
$quStrGetAgencies1 = "SELECT * FROM AGENCIES WHERE GID=$grid";
        $quGetAgencies1 = mysqli_query($GLOBALS['dbh'], $quStrGetAgencies1) or die($quStrGetAgencies1);
...
<?php
    ...
    $quGetname = mysqli_query($GLOBALS['dbh'], $quStrGetname) or die($quStrGetname);
...
while ($rowGetname = mysqli_fetch_object($quGetname)) {?>
        ...
        ?>
				...
				<?php if (isset($switch_remember['rented_by']) and is_array($switch_remember['rented_by'])) {
...
<NOBR>Pending <img src="../assets/images/icons/pending-yes.gif" border=0 HEIGHT="16" WIDTH="16" alt="Pending Status - Yes" title="Pending Status - YES - Check Status">:<input type="checkbox" name="status_pending[]" value="1" <?php if (isset($switch_remember['status_pending'])) {echo " checked ";}?> ></NOBR>
</DIV></TD>
-->
...
<?php }?>
	...
	<table border="0" cellspacing="0" cellpadding="0" width = "100%" height="10" BGCOLOR="lightgrey">
		<tr>
		<td width="230" align="center">
		<form action="<?php echo "$PHP_SELF"; ?>" method="GET" name="listings_view_form">
            <input type="hidden" name="op" value="listings">
            <input type="hidden" name="listing_filter_display" value="<?php echo $listing_filter_display ?>">
            <input type="hidden" name ="start" value="<?php echo $start; ?>">
            <input type="hidden" name ="limitN" value="<?php echo $limitN; ?>">
...
while ($rowGetViews = mysqli_fetch_object($quGetViews)) {?>
			<?php if ($rowGetViews->ID != 9 || $_SESSION["agcy"] > 0) {?>
		...
		<td width="1">&nbsp;</td>
		<td align="center"><form action="<?php echo "$PHP_SELF"; ?>" method="GET" name="limitNForm"><input type="hidden" name="op" value="listings"><div class="controltext">Show<input type="text" size="2" name="limitN" value="<?php echo $limitN; ?>" onchange="JavaScript:document.limitNForm.submit();">listings per page.</div>
                                <input type="hidden" name ="start" value="<?php echo $start; ?>">
            <input type="hidden" name ="vid" value="<?php echo $vid; ?>"></form></td>
		<td width="1">&nbsp;</td>
		<td align="center">
...
if ($userFilter) {?>
		    ...
		    <?php } else {?>
		    	<a href="<?php echo "$PHP_SELF?op=listings&start=z&userFilter=1&filterChange=1"; ?>">Show only <?php echo $_SESSION["handle"]; ?>'s listings</a>
		    <?php }?>
		   -
		    <?php
if ($activeFilter) {?>
...
if (isset($switch_remember["landlord"]) and $switch_remember["landlord"] != "") {

    error_reporting(0);

    $quStrGetLLN = "SELECT SHORT_NAME FROM LANDLORD WHERE `GRID`=$grid AND `LID`=$switch_remember[landlord] LIMIT 1";
    $quGetLLN = mysqli_query($GLOBALS['dbh'], $quStrGetLLN);
    $rowGetLLN = mysqli_fetch_object($quGetLLN);

}

if (isset($rowGetLLN) and $rowGetLLN->SHORT_NAME != "") {
    ?>

<?php if ($user_level > "0.5") {
        ?>
<NOBR><?php echo "<font size=-1><a href=\"$PHP_SELF?op=editLandlord&lid=$switch_remember[landlord]\" target=\"_editll\"><img border=0 src=\"../images/icons/edit.gif\" alt=\" Edit $rowGetLLN->SHORT_NAME\" TITLE=\"Edit $rowGetLLN->SHORT_NAME\">Edit $rowGetLLN->SHORT_NAME</A> | </FONT>"; ?>


	 <A HREF="<?php echo "$PHP_SELF?op=adlEdit&LLID=$switch_remember[landlord]"; ?>" target="_LLNL"><img border="0" vspace="0" hspace="0" SRC="../images/edit-new-listing-ll.gif" ALT="Create a new listing for <?php echo $rowGetLLN->SHORT_NAME; ?>" TITLE="Create A New Listing For <?php echo $rowGetLLN->SHORT_NAME; ?>" ALT="Create A New Listing For This Landlord"> <FONT SIZE=-1>Create a new listing for <?php echo $rowGetLLN->SHORT_NAME; ?></a>
 | <a href="<?php echo $PHP_SELF; ?>?op=changeLLListingsdo&amp;lid=<?php echo $switch_remember[landlord]; ?>&amp;grid=<?php echo $grid; ?>&amp;next_contact=<?php echo $rowGetLandlord->NEXT_CONTACT; ?>">Set All <?php echo $rowGetLLN->SHORT_NAME; ?> Listings to Updated Today <img border="0" vspace="0" hspace="0" src="https://www.BostonApartments.com/images/newIcon.gif" alt="Mark Listings as New" title="Mark Listings as New"></A></NOBR>
<BR><NOBR>

<?php
echo "<A HREF=$PHP_SELF?op=buildings&lid=$switch_remember[landlord]&llname=$rowGetLLN->SHORT_NAME><IMG src=../assets/images/buildings.jpg BORDER=0 TITLE=\"Edit Landlord's Units By Building Globally/Individually\">Edit Landlord's Units By Building Globally/Individually </A> | <A HREF=$PHP_SELF?op=global-Landlord&lid=$switch_remember[landlord]&llname=$rowGetLLN->SHORT_NAME><IMG src=../assets/images/global.jpeg BORDER=0 TITLE=\"Make Global changes to ALL Listings for $rowGetLLN->SHORT_NAME\">Make Global changes to ALL Listings for $rowGetLLN->SHORT_NAME</A>";
        ?>
</NOBR><BR>
<?php }?>
...
</TD><TD VALIGN="TOP" ALIGN="CENTER" BGCOLOR="lightgrey">
<?php if ($user_level > 0) {?>
		      ...
		      <form action="<?php echo "$PHP_SELF?op=select_and_do&return_page=listings"; ?>" method="POST" name="moveform">
		      	...
		      	<div class="input-group p-2">
		      		      <select class="form-control" size="1" name="sop" style="background-color:white">
		      			...
		      			<?php if ($user_level >= 3) {echo "<option value='delete'>Delete...</option>";}?>
		      				...
		      				<option value="priority">Mark  Priority Listing</option>
		      		  		<option value="nofee">Make NO FEE</option>
		      		  		<option value="fee">Make FULL FEE</option>
		      		  		<option value="feeprint">Make FULL FEE (print)</option>
		      		  		<option value="fee2">Make FEE</option>			  		<option value="fee2print">Make FEE (print)</option>
		        ...
		        <div class="input-group-prepend">
		          <button type="submit" class="btn btn-outline-secondary" NAME = "SUBMIT" type="button">Execute</button>
		        </div>
		      </div>
...
<?php if ($vid == 9 && $_SESSION["agcy"] > 0) {
    // AGENCY ?>
	<FONT SIZE="-3"><br></FONT>
        <center>
        <?php if ($_SESSION["isAdmin"] || $_SESSION["handle"] == "eboyer") {
        ?>
...
<?php }?>
	...
	<?php if (isset($rowGetView['GRID_EDIT'])) {?>
    ...
    ?>
		...
		<td class="controltext" width="<?php echo isset($col[2]) ? $col[2] : ''; ?>" height="28" bgcolor="<?php echo $coltitcolor; ?>">



<?php if (isset($col[3]) and $col[3] == 'ckbox') {?>
<CENTER>
<span style="font-size:7px;"><NOBR>&#9745;All</NOBR><input type="checkbox" name="allbox" value="sel_all" onClick="CheckAll();"></span></CENTER>

<?php } elseif (isset($col[1]) and $col[1] == 'delete') {?>

<span style="font-size:8px;">Del</span></CENTER>

<?php } elseif (isset($col[1]) and $col[1] == 'edit') {?>
...
<CENTER><span style="font-size:6px;">Client Sheet</span></CENTER>
                        ...
                        <?php } elseif (isset($col[0]) and $col[0] == 'csheetpic') {?>

<CENTER><span style="font-size:6px;">Client Sheet</span></CENTER>



                        <?php } elseif (isset($col[1]) and $col[1] == 'email') {?>

<span style="font-size:8px;">Email</span></CENTER>



                        <?php } elseif (isset($col[0]) and $col[0] == 'adprint') {?>

<CENTER><span style="font-size:7px;">Print Ad</span></CENTER>

                        <?php } elseif (isset($col[0]) and $col[0] == 'weblist') {?>
...
<span style="font-size:8px;">Website</SPAN><span style="font-size:6px;"><BR></SPAN><span style="font-size:8px;"><NOBR>Web List</NOBR></span></CENTER>
                        ...
                        <?php } elseif (isset($col[0]) and $col[0] == 'pending') {?>
...
<a href="<?php echo "$PHP_SELF?op=listings&sort=STATUS_PENDING&sortD=$antiSortD"; ?>"><font color="<?php echo $rowGetView['COLOR1']; ?>"><?php echo "<span style=\"font-size:9px;\">Pndg</FONT>"; ?><?php echo $sortImage; ?></span></a>
                        ...
                        <?php } elseif (isset($col[0]) and ($col[0] == 'descriptive' or $col[0] == 'descriptive-agent')) {?>
...
<a href="<?php echo "$PHP_SELF?op=listings&sort=" . (isset($col[1]) ? $col[1] : "") . "&sortD=$antiSortD"; ?>"><font color="<?php echo $rowGetView['COLOR1']; ?>"><?php echo isset($col[3]) ? $col[3] : ""; ?><?php echo $sortImage; ?></font></a>
...
<?php }?>
    ...
    ?>
        ...
        <?php if (isset($rowColor) and $rowColor == "#FFFFFF") {
		...
		<tr tabindex="2" id="ad<?php echo $k++; ?>">
		<?php
$i = 0;
    foreach ($cols as $col) {
        ?>

			<td class="ad" width="<?php echo isset($col[2]) ? $col[2] : ''; ?>" height="28" bgcolor="<?php echo $rowColor; ?>" nowrap>
			<?php if ($col[0] == 'f') {
            ...
            if (isset($col[5])) {
                ...
                //use values;
                if ($col[4] == "df") {
                    if (isset($DEFINED_VALUE_SETS[$col[1]][$rowGetListings[$col[1]]])) {
            ...
            if ($col[4] == 'text') {
                ...
                echo "<input type=hidden name=gid value=" . (isset($gid) ? $gid : "") . ">";
...
<?php if ($key == $rowGetListings[$col[1]]) {echo " selected ";}
                    ?>
                                        >
...
if (!$rowGetListings['PARKING_NUM']) {
            ...
            ?>
				                       ...
				                       <?php } elseif ($col[0] == 'mbta') {?>

				<CENTER><A HREF="http://mbta.com/rider_tools/servicenearby/?saServiceNearBy=<?php echo $rowGetListings["STREET_NUM"]; ?>+<?php echo $rowGetListings["STREET"]; ?>%2C+<?php echo $rowGetListings["ZIP"]; ?>&sLocationServiceNearBy" target="_MBTA"><img src="../assets/images/mbta.gif" vspace="0" hspace="0" border="0" height="16" width="16" alt="MBTA" title="MBTA"></A></CENTER>
...
<?php
            ...
            if ($rowGetListings['CSOURCE'] == "1") {echo "<FONT COLOR=BLUE><B>ZR</B></FONT>";} elseif ($rowGetListings['CSOURCE'] == "2") {echo "<FONT COLOR=BLUE><B>YGL</B></FONT>";} elseif ($rowGetListings['CLI'] != '1075' and $rowGetListings['CLI'] != $grid and $rowGetListings['CLI'] != '392' and $rowGetListings['CSOURCE'] == "0") {
                echo "<FONT COLOR=BLUE><B>BA</B></FONT>";} elseif

            ($rowGetListings['CLI'] == "1075" and $rowGetListings['STATUS_ACTIVE'] == "1") {echo "<FONT COLOR=BLUE><B>MLS</B></FONT>";} elseif
            ($rowGetListings['CLI'] == "1075" and $rowGetListings['STATUS_ACTIVE'] == "0") {echo "<FONT COLOR=GRAY><B>MLS</B></FONT>";} else {}

            if (($rowGetListings['CSOURCE'] == "0" or $rowGetListings['CSOURCE'] == "1" or $rowGetListings['CSOURCE'] == "2") and ($rowGetListings['CLI'] == $grid)) {
...
<img src="../assets/images/inact.jpg" border=0 HEIGHT="16" WIDTH="16" alt="Deactivated Ad - Click to Advertise This Listing" title="Deactivated Ad - Click to Advertise This Listing">
<?php if ($user_level > 0) {?>
</a>
<?php }?>
...
<?php }?>
<img src="../assets/images/act.gif" border=0 HEIGHT="16" WIDTH="16" alt="Activated Ad - Click to shut off the Ad" title="Activated Ad">
<?php if ($user_level > 0) {?>
</a>
<?php }?>
			<?php	} else {echo "ERROR";}}?>
...
<?php if ($rowGetListings['CLI'] == "$grid" and $user_level > 0) {?>
...
<?php }?>
<img src="../assets/images/icons/a.jpg" border=0 HEIGHT="16" WIDTH="16" alt="Available Listing - Click to Mark Unavailable" title="Available Listing - Click to Mark Unavailable">
<?php if ($rowGetListings['CLI'] == "$grid" and $user_level > 0) {?>
</a>
<?php }?>
...
<?php }?>
<img src="../assets/images/icons/u.jpg" border=0 HEIGHT="16" WIDTH="16" alt="Unavailable Listing - Click to Mark Available" title="Unavailable Listing - Click to Mark Available">
<?php if ($rowGetListings['CLI'] == "$grid" and $user_level > 0) {?>
</a>
<?php }?><?php }?></FONT>
</TD>
...
<?php if ($rowGetListings['PIC'] != 0) {
            ...
            } else { $thumb = "<FONT SIZE=-2 color=#BDBDBD>No Photo</FONT>";}

            if ($rowGetListings['THUMBNAIL']) {

                if ($rowGetListings['CLI'] == '1075') {
            ...
            if ($rowGetListings['PIC'] > 0) {echo "<FONT SIZE=-1>x " . $rowGetListings['PIC'] . "</FONT>";} elseif ($rowGetListings['EXTERNALPIC_NUM'] > 0) {echo "<FONT SIZE=-1>x " . $rowGetListings['EXTERNALPIC_NUM'] . "</FONT>";}
...
if ($rowGetListings['STREET_NUM'] != "" and $rowGetListings['STREET'] != "") {
                    ...
                    $result3 = mysqli_query($GLOBALS['dbh'], $quStrGetstatusp);
...
<?php
if (isset($rowGetListings['BATHBATH']) and $rowGetListings['BATHBATH'] == '0.00') {
                echo "";
            } elseif (isset($rowGetListings['BATH']) and $rowGetListings['BATH'] == '99.00') {
                echo "Shared";
            } else {
...
// RJ
            ...
            if ($rowGetListings["CSOURCE"] == "1") {
                if ($rowGetListings["MLS_AGENCY"] != "") {echo "<NOBR><FONT COLOR=RED><B>Agency:</B></FONT> <FONT COLOR=BLUE><B>" . $rowGetListings["MLS_AGENCY"] . "</B></FONT></NOBR><BR>";}
                if ($rowGetListings["MLS_AGENT"] != "") {echo "<NOBR><FONT COLOR=RED></B>Listing Agent:</B></FONT> <FONT COLOR=BLUE><B>" . $rowGetListings["MLS_AGENT"] . "</B></FONT></NOBR><BR>";}
                if ($rowGetListings["MLS_CONTACT"] != "") {echo "<NOBR><FONT COLOR=RED>Email:</B></FONT> <FONT COLOR=BLUE><B><A HREF=mailto:" . $rowGetListings["MLS_CONTACT"] . ">" . $rowGetListings["MLS_CONTACT"] . "</A></B></FONT></NOBR><BR>";}
                if (($rowGetListings["MLS_PHONE"] != "") and ($rowGetListings["MLS_PHONE"] != $rowGetListings["MLS_CONTACT"])) {echo "<NOBR><FONT COLOR=RED>Listing Phone:</B></FONT> <FONT COLOR=BLUE><B>" . $rowGetListings["MLS_PHONE"] . "</B></FONT></NOBR><BR>";}
                if (($rowGetListings["MLS_PHONEOFFICE"] != "")) {echo "<NOBR><FONT COLOR=RED>Listing Office Phone:</B></FONT> <FONT COLOR=BLUE><B>" . $rowGetListings["MLS_PHONEOFFICE"] . "</B></FONT></NOBR><BR>";}
                if (($rowGetListings["EXTERNALID"] != "")) {echo "<NOBR><FONT COLOR=RED>Zillow ID #:</B></FONT> <FONT COLOR=BLUE><B>" . $rowGetListings["EXTERNALID"] . "</B></FONT></NOBR><BR>";}

// end RJ
            }

            if ($rowGetListings["CSOURCE"] == "2") {
                if ($rowGetListings["MLS_AGENCY"] != "") {echo "<NOBR><FONT COLOR=RED><B>Owner:</B></FONT> <FONT COLOR=BLUE><B>" . $rowGetListings["MLS_AGENCY"] . "</B></FONT></NOBR><BR>";}
                if ($rowGetListings["MLS_AGENT"] != "") {echo "<NOBR><FONT COLOR=RED></B>Listing Agent:</B></FONT> <FONT COLOR=BLUE><B>" . $rowGetListings["MLS_AGENT"] . "</B></FONT></NOBR><BR>";}
                if ($rowGetListings["MLS_CONTACT"] != "") {echo "<NOBR><FONT COLOR=RED>Email:</B></FONT> <FONT COLOR=BLUE><B><A HREF=mailto:" . $rowGetListings["MLS_CONTACT"] . ">" . $rowGetListings["MLS_CONTACT"] . "</A></B></FONT></NOBR><BR>";}
                if (($rowGetListings["MLS_PHONE"] != "") and ($rowGetListings["MLS_PHONE"] != $rowGetListings["MLS_CONTACT"])) {echo "<NOBR><FONT COLOR=RED>Listing Phone:</B></FONT> <FONT COLOR=BLUE><B>" . $rowGetListings["MLS_PHONE"] . "</B></FONT></NOBR><BR>";}
                if (($rowGetListings["MLS_PHONEOFFICE"] != "")) {echo "<NOBR><FONT COLOR=RED>Listing Office Phone:</B></FONT> <FONT COLOR=BLUE><B>" . $rowGetListings["MLS_PHONEOFFICE"] . "</B></FONT></NOBR><BR>";}
                if (($rowGetListings["EXTERNALID"] != "")) {echo "<NOBR><FONT COLOR=RED>YGL ID #:</B></FONT> <FONT COLOR=BLUE><B>" . $rowGetListings["EXTERNALID"] . "</B></FONT></NOBR><BR>";}
            }

            if ($rowGetListings['CLI'] != '1075' and $rowGetListings['CLI'] != $grid and $rowGetListings['CLI'] != '392' and $rowGetListings['CSOURCE'] == '0') {

                echo "<B>Contact:</B> <FONT COLOR=BLUE>";

                if ($rowGetListings['ALTSIG'] != "") {echo $rowGetListings['ALTSIG'];} else {

                    $qustrgetsig = "SELECT `SIG` FROM `GROUP` WHERE `GRID`='$rowGetListings[CLI]' LIMIT 1";
                    @$qugetsig = mysqli_query($GLOBALS['dbh'], $qustrgetsig);
                    $rowgetsig = mysqli_fetch_object($qugetsig);

                    echo $rowgetsig->SIG . "</FONT>";
                }
...
<?php if ($rowGetListings["LAST_MOD_BY"]) {
                    ?>

by <?php echo $rowGetListings["LAST_MOD_BY"];
...
<?php if ($rowGetListings['NOFEE'] == "1" or $rowGetListings['NOFEE'] == "3") {echo "<FONT COLOR=red>NO FEE</FONT>";}?>
...
<?php if ($rowGetListings['NOFEE'] == "7" or $rowGetListings['NOFEE'] == "8" or $rowGetListings['NOFEE'] == "9" or $rowGetListings['NOFEE'] == "10") {echo "FEE";}?>
...
<?php
if ($rowGetListings["FEE_COMMENTS"] != "") {
                echo "<BR>" . $rowGetListings["FEE_COMMENTS"] . "<BR>";
            }
            ?>
<BR>
...
<?php
if ($rowGetListings['PETSA'] == "") {echo "";} elseif ($rowGetListings['PETSA'] == "1") {echo "<NOBR><FONT COLOR=red>N0 PETS</FONT></NOBR>";} elseif ($rowGetListings['PETSA'] == "2") {echo "<NOBR>small pet</NOBR>";} elseif ($rowGetListings['PETSA'] >= "3" and $rowGetListings['PETSA'] < "4") {echo "<NOBR>Cat OK</NOBR>";} elseif ($rowGetListings['PETSA'] == "4") {echo "<NOBR>Small Dog</NOBR>";} elseif ($rowGetListings['PETSA'] == "5") {echo "<NOBR>Pets OK</NOBR>";} elseif ($rowGetListings['PETSA'] == "6") {echo "<NOBR>Pet Friendly</NOBR>";} elseif ($rowGetListings['PETSA'] == "7") {echo "<NOBR>Negotiable</NOBR>";} elseif ($rowGetListings['PETSA'] == "8") {echo "<NOBR>Dogs OK</NOBR>";} elseif ($rowGetListings['PETSA'] == "9") {echo "<NOBR>Dogs Breed Rules</NOBR>";}
            ?>
...
<?php
if (($rowGetListings['FEATURES_DELEADED'] == "1") and ($rowGetListings['TYPE'] == "1")) {echo "<nobr>Lead Safe</nobr>";}
            ?>
<?php
if (($rowGetListings['NOT_DELEADED'] == "1") and ($rowGetListings['TYPE'] == "1")) {echo "<nobr>Not Deleaded</nobr>";}
            ?>
...
</TD></TR><TR><TD ALIGN=LEFT><NOBR><FONT SIZE="-1">Avail:&nbsp;</FONT></TD><TD ALIGN=RIGHT><FONT SIZE="-1"> <B><?php echo fuzDate($rowGetListings['AVAIL']); ?></B></FONT></NOBR></TD></TR><TR><TD ALIGN=LEFT><FONT SIZE="-1">
<NOBR>Modified:</NOBR></FONT></TD><TD ALIGN=RIGHT><NOBR><FONT SIZE="-1"> <?php echo $rowGetListings['MOD']; ?> </FONT></NOBR></TD></TR>
...
<?php
            ...
            if ($col[0] == 'descriptive') {
                ...
                $quGetAgentAd = mysqli_query($GLOBALS['dbh'], $quStrGetAgentAd);
                $rowGetAgentAd = mysqli_fetch_object($quGetAgentAd);

                if (isset($rowGetAgentAd->BODY_AGENT)) {
            ...
            if ($rowGetListings['AUTO_WRITE'] != "1") {
                ...
                if ($rowGetListings['FEATURES_INTERCOM'] == "1") {echo " | Intercom ";}
                ...
                if ($rowGetListings['FEATURES_STAINLESS'] == "1") {echo " | Stainles Steel Appliances ";}
                if ($rowGetListings['FEATURES_GRANITEC'] == "1") {echo " | Granite Countertops ";}
                ...
                if ($rowGetListings['FEATURES_BAYWINDOWS'] == "1") {echo " | Bay Windows ";}
                ...
                if ($rowGetListings['AMENITIES_BIKEROOM'] == "1") {echo " | Bike Storage ";}
                ...
                if ($rowGetListings['AMENITIES_PLAYGROUND'] == "1") {echo " | Playground ";}
                if ($rowGetListings['AMENITIES_BASKETBALL'] == "1") {echo " | Basketball Court ";}
                if ($rowGetListings['AMENITIES_BBQ'] == "1") {echo " | BBQ ";}
                if ($rowGetListings['FEATURES_CABLETV'] == "1") {echo " | Cable TV ";}
                if ($rowGetListings['FEATURES_CEILINGFAN'] == "1") {echo " | Ceiling Fan ";}
                ...
                if ($rowGetListings['AMENITIES_ZIPCAR'] == "1") {echo " | Zip Car ";}
...
<CENTER>
                        ...
                        <?php if (($user_level >= "3") or ($user_level < "3" and $rowGetListings['STATUS'] == "ACT")) {
                ?>

<a href="<?php echo "https:///www.bostonapartments.com/plugin/?ad=" . $rowGetListings['CID'] . "&uid=$uid&cli=" . $rowGetListings['CLI'] . "\" target=_cl" . $rowGetListings['CID'] . "><img width=\"16\" height=\"16\" border=\"0\" vspace=\"0\" hspace=\"0\" src=\"../images/icons/cl.gif\" alt=\"Post to Craigslist\" title=\"Post to Craigslist\"></a>";

// <a href="<?php echo "https://www.BostonApartments.com/clpost.php?ad=".$rowGetListings['CID']."&cli=".$rowGetListings['CLI']."&uid=$uid"."\" target=\"_CL".$rowGetListings['CID']."\"";                                                                                                                                                                                             ?><!-- "><img width="16" height="16" border="0" vspace="0" hspace="0" src="../images/icons/cl.gif" alt="Post to Craigslist" title="Post to Craigslist"></A><BR>
-->
<?php }?>

<BR>
<!-- New facebook share /alexander -->
<div class="fb-share-button" data-href="https://bostonapartments.com/listing.php?ad=<?=$rowGetListings['CID']?>" data-layout="button" data-size="small" data-mobile-iframe="true"><a target="_blank" href="https://www.facebook.com/sharer/sharer.php?u=https%3A%2F%2Fbostonapartments.com%2Flisting.php%3Fad%3D<?=$rowGetListings['CID']?>&amp;src=sdkpreparse" class="fb-xfbml-parse-ignore">Share</a></div>
...
<?php
            ...
            if ($rowGetListings['AD_TITLE'] != "") {
                $twitter_title = $rowGetListings['AD_TITLE'];
            } else {
                $twitter_title = $rowGetListings['LOCNAME'] . " - $" . $rowGetListings['PRICE'] . $rowGetListings['ROOMS'] . "%20Bed";
            }
            ?>

<link rel="canonical"
  href="https://www.bostonapartments.com/listing.php?ad=<?php echo $rowGetListings['CID'] ?>">


<a class="twitter-share-button"
  href="https://twitter.com/intent/tweet?url=https%3A%2F%2Fwww.bostonapartments.com%2Flisting.php%3Fad%3D<?php echo $rowGetListings['CID'] ?>" target="_tweet"
  data-size="small"
  data-text="<?php echo $twitter_title; ?>"
  data-url="https%3A%2F%2Fbostonapartments.com%2Flisting.php%3Fad%3D<?php echo $rowGetListings['CID']; ?>"
  data-hashtags="bostonapartments,apartments,realestate"
  data-via="BostonApts.com"
  data-related="realestate,apartments">
<img width="16" height="16" border="0" vspace="0" hspace="0" src="../assets/images/twitter.jpeg" alt="Post to Twitter" title="Post to Twitter">
</a>
...
<TD VALIGN="TOP">
<CENTER>
<a href="/lacms/clients/AgencyArea2.php?op=adlEdit&cid=<?php echo $rowGetListings['CID']; ?>" target="_new<?php echo $rowGetListings['CID']; ?>" TITLE="Edit"><img border=0 src="../images/icons/edit.gif" alt="Edit" title="Edit"></a><BR>
<FONT SIZE="-3">&nbsp;<BR></FONT>
...
<FONT SIZE="-3">&nbsp;<BR></FONT>
<A HREF="http://mbta.com/rider_tools/servicenearby/?saServiceNearBy=<?php echo $rowGetListings["STREET_NUM"]; ?>+<?php echo $rowGetListings["STREET"]; ?>%2C+<?php echo $rowGetListings["ZIP"]; ?>&sLocationServiceNearBy" target="_MBTA"><img src="../assets/images/mbta.gif" vspace="0" hspace="0" border="0" height="16" width="16" alt="MBTA" title="MBTA"></A><BR>
...
<?php if ($rowGetListings['CLI'] == $grid) {?>
...
<?php }?>
            ...
            if ($rowGetListings['PETSA'] == "") {echo "";} elseif ($rowGetListings['PETSA'] == "1") {echo "<FONT COLOR=red>N0 PETS</FONT>";} elseif ($rowGetListings['PETSA'] == "2") {echo "small pet";} elseif ($rowGetListings['PETSA'] >= "3" and $rowGetListings['PETSA'] < "4") {echo "Cat OK";} elseif ($rowGetListings['PETSA'] == "4") {echo "Small Dog";} elseif ($rowGetListings['PETSA'] == "5") {echo "Pets OK";} elseif ($rowGetListings['PETSA'] == "6") {echo "Pet Friendly";} elseif ($rowGetListings['PETSA'] == "7") {echo "Negotiable";}
            ?>
...
<?php }?>
      ...
      <?php } elseif (($col[0] == 'cl') and (($user_level >= "3") or ($user_level < "3" and $rowGetListings['STATUS'] == "ACT"))) {?>



<CENTER><a href="<?php echo "https:///www.bostonapartments.com/plugin/?ad=" . $rowGetListings['CID'] . "&uid=$uid&cli=" . $rowGetListings['CLI'] . "\" target=_cl" . $rowGetListings['CID'] . "><img width=\"16\" height=\"16\" border=\"0\" vspace=\"0\" hspace=\"0\" src=\"../images/icons/cl.gif\" alt=\"Post to Craigslist\" title=\"Post to Craigslist\"></a>"; ?>
...
<!-- <a href="<?php echo "http://www.facebook.com/sharer.php?u=http%3A%2F%2Fbostonapartments.com%2Fhomepage-fb.php%3Fcli%3D" . $rowGetListings['CLI'] . "%26ad%3D" . $rowGetListings['CID']; ?>" target="_face<?php echo $rowGetListings['CID']; ?>"><img width="16" height="16" border="0" vspace="0" hspace="0" src="../images/icons/facebook.gif" alt="Post to Facebook" title="Post to Facebook"></A><BR> -->
...
$thisYear = date("Y");
            for ($i = 0; $i <= 4; $i++) {?>
				...
				<?php }?>
                        ...
                        <?php } elseif ($col[0] == 'copy') {?>


	<?php if ($user_level > "0" and ($rowGetListings['CLI'] == "$grid" or $rowGetListings['CLI'] == "1075")) {?>

<a href="<?php echo $PHP_SELF . "?op=copy&cid=" . $rowGetListings['CID']; ?>"><font size=-1>Copy</font><a>
...
<?php if ($rowGetListings['NOFEE'] == "1" or $rowGetListings['NOFEE'] == "3") {echo "<CENTER><FONT COLOR=red>NO FEE</FONT></CENTER>";}?>
...
<?php if ($rowGetListings['NOFEE'] == "7" or $rowGetListings['NOFEE'] == "8" or $rowGetListings['NOFEE'] == "9" or $rowGetListings['NOFEE'] == "10") {echo "<CENTER>FEE</CENTER>";}?>
</TD></TR></TABLE>
...
<?php
            ...
            if ($rowGetListings['CSOURCE'] == "1") {echo "<FONT COLOR=BLUE><B>ZR</B></FONT>";}
            if ($rowGetListings['CSOURCE'] == "2") {echo "<FONT COLOR=BLUE><B>YGL</B></FONT>";}
...
<img src="../assets/images/inact.jpg" border=0 alt="Deactivated Ad -  - Click to Advertise This Listing" title="Deactivated Ad - Click to Advertise This Listing">
<?php if ($user_level > 0) {?>
</a>
<?php }?></CENTER>
				<?php } elseif ($rowGetListings['STATUS'] == "ACT") {?>
...
<?php if ($user_level > 0) {?>
				<a href="<?php echo "$PHP_SELF?op=deactivate_a&cid=" . $rowGetListings['CID'] . "&return_page=listings&return_page_div=" . $k; ?>">
<?php }?>
<img src="../assets/images/act.gif" border=0 alt="Activated Ad - Click to shut off the Ad" title="Activated Ad - Click to shut off the Ad">
<?php if ($user_level > 0) {?>
</a>
<?php }?>
			<?php	} else {echo "ERROR";}}?>
...
<?php if ($user_level > 0) {?>
                              ...
                              <a href="<?php echo "$PHP_SELF?op=mark_status_active&cid=" . $rowGetListings['CID'] . "&turn=unavailable&return_page=listings&return_page_div=" . $k; ?>">
<?php }?>
<img src="../assets/images/icons/a.jpg" border=0 alt="Available Unit - Click to Mark Unavailable" title="Available Unit - Click to Mark Unavailable">
<?php if ($user_level > 0) {?>
</a>
<?php }?>
</CENTER>
...
<?php if ($user_level > 0) {?>
			      <a href="<?php echo "$PHP_SELF?op=mark_status_active&cid=" . $rowGetListings['CID'] . "&turn=available&return_page=listings&return_page_div=" . $k; ?>">
<?php }?>
<img src="../assets/images/icons/u.jpg" border=0 alt="Unavailable Unit - Click to Mark Available" title="Unavailable Unit - Click to Mark Available">
<?php if ($user_level > 0) {?>
</a>
<?php }?>
</CENTER>
	...
	<table border="0" width = "100%" cellspacing="0" cellpadding="0" height="10" BGCOLOR="lightgrey">
		<tr><FORM></FORM>
		<td width="230" align="center" VALIGN="TOP">
		<form action="<?php echo "$PHP_SELF"; ?>" method="GET" name="listings_view_form2">
            <input type="hidden" name="op" value="listings">
            <input type="hidden" name="listing_filter_display" value="<?php echo $listing_filter_display; ?>">
            <input type="hidden" name ="start" value="<?php echo $start; ?>">
            <input type="hidden" name ="limitN" value="<?php echo $limitN; ?>">
    ...
    if (!$thisCalc) {
		...
		<div class="controltext">
		 <a href="<?php echo "$PHP_SELF?op=listings&start=z&limitN=$num_rows&filterChange=1"; ?>">Show all listings</a> -
		    <?php
if ($userFilter) {?>
		    	<a href="<?php echo "$PHP_SELF?op=listings&start=z&userFilter=n"; ?>">Show all listings for <?php echo $_SESSION["group"]; ?></a>
		    <?php } else {?>
		    	<a href="<?php echo "$PHP_SELF?op=listings&start=z&userFilter=1"; ?>">Show only <?php echo $_SESSION["handle"]; ?>'s listings</a>
		    <?php }?>
		   -
		    <?php
if ($activeFilter) {?>
