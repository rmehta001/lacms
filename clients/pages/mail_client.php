<?php
if (isset ($clid)) if($clid !="") {
$action = "Client emailed by $_SESSION[handle]";
...
<input type="text" name="to" value="<?php if(isset($rowGetUser)) echo "$rowGetClient->CLIENT_EMAIL";?>" /><br />
...
<input type="text" name="subject" value="Email from <?php if(isset($rowGetUser)) echo "$rowGetUser->FNAME";?> <?php if(isset($rowGetUser)) echo "$rowGetUser->LNAME";?>" />
...
<?php if(isset($email_head)) echo $email_head; ?>
<BR><BR>
Dear <?php if(isset($rowGetClient)) echo "$rowGetClient->NAME_FIRST";?>,<BR><BR>
...
<?php if(isset($rowGetUser)) echo "$rowGetUser->POSITION";?><BR>
<?php }?>
...
<BR><BR>
				...
				<td align="center" height="30" bgcolor="#FFFF99"><select name="client_list" multiple="multiple" size="10" style="width:200px">
					...
					<?php while ($rowGetClients = mysqli_fetch_object($quGetClients)) {?>
						<option value="<?php if(isset($rowGetClients)) echo $rowGetClients->CLID;?>"><?php if(isset($rowGetClients)) echo $rowGetClients->NAME_LAST;?>, <?php if(isset($rowGetClients)) echo $rowGetClients->NAME_FIRST;?> - <?php if(isset($rowGetClients)) echo $rowGetClients->CLIENT_EMAIL;?></option>
					<?php } ?>
					</select>
				</td>
