...
exit;
	...
	foreach ($pids as $pid) {
		...
		$quGetPicture = mysqli_query($GLOBALS['dbh'], $quStrGetPicture) or die (mysqli_error());
		...
		$quDeletePicture = mysqli_query($GLOBALS['dbh'], $quStrDeletePicture) or die (mysqli_error());
	
		$quStrUpdateListing = "update CLASS set PIC=PIC-1 where CID='$cid'";
		$quUpdateListing = mysqli_query($GLOBALS['dbh'], $quStrUpdateListing) or die (mysqli_error());
	}
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" bgcolor="#FFFFFF">
	...
	<!--	<input type="hidden" name="MAX_FILE_SIZE" value="81920000000"> -->
...
if (isset($rowGetGroup->PIC_CUSTOM_WIDTH)) {
?>

Large<input type="radio" name="picsize" id="picsize" value="1" checked> Small<input type="radio" name="picsize" id="picsize" value="0"> Custom Width:<input type="radio" name="picsize" id="picsize" value="2" checked> <input id="pic_custom_width" type="text" name="pic_custom_width" size="5" value="<?php echo $rowGetGroup->PIC_CUSTOM_WIDTH;?>"></span>

<?php } else { ?>

Large<input type="radio" name="picsize" id="picsize" value="1"> Small<input type="radio" name="picsize" id="picsize" value="0"> Custom Width (px): <input type="radio" name="picsize" id="picsize" value="2"> <input id="pic_custom_width" type="text" name="pic_custom_width" size="5" value="">

<?php } ?>

 Watermark:  <NOBR><input type="checkbox" name="watermarkon" <?php if  ($_SESSION["watermark_on"]=="1") {echo " checked ";} ?>></SPAN> | <FONT SIZE="-2"><A HREF="<?php echo "$PHP_SELF?op=watermarkprefs"; ?>" target="_NEW">Set Watermark Preferences</A></FONT>&nbsp;</NOBR></DIV>
...
<?php if ($user_level>0) {?>
<a href="<?php echo "pages/managePics-reorder.php?cid=$cid"; ?>"><BUTTON TYPE="BUTTON" STYLE="Background-Color : #FFFF99;">Change Picture Order</BUTTON></a></NOBR><BR>
<?php } ?>
