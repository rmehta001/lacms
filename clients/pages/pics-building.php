<!--BEGIN building Pics Photo Gallery -->

<BR><CENTER>

<TABLE BGCOLOR="#FFFFFF" BORDER="1" BORDERCOLOR="#000000" CELLPADDING="0" CELLSPACING="0"><TR><TD>

<TABLE BGCOLOR="#FFFFFF" BORDER="0"><TR><TD VALIGN="MIDDLE" ALIGN="CENTER">
<a href="<?php echo "$PHP_SELF?op=pics-building&street_num=$rowGetAd->STREET_NUM&street=$rowGetAd->STREET";?>" target="_NEW"><img border="0" src="../assets/images/pic-gallery.jpeg" HEIGHT="79" WIDTH="112"></a></TD><TD VALIGN="MIDDLE" ALIGN="CENTER">
<B><FONT SIZE="+2">
<a href="<?php echo "$PHP_SELF?op=pics-building&street_num=$street_num&street=$street";?>" target="_NEW">Building Photo Gallery for <?php echo $street_num ;?>  <?php echo $street ;?></A> &nbsp;
</FONT></B><BR>
<B><A HREF="<?php echo "$PHP_SELF?op=upload1tomany-b&street_num=$street_num&street=$street&lid=$lid&return_page=pics-building";?>"><FONT COLOR="GREEN">Upload 1 Picture to ALL UNITS @ <?php echo "$street_num $street";?></FONT> - <FONT  SIZE="-3" COLOR="#999999">Click Here</FONT></A></B><BR>
...
$quStrGetBPics = "SELECT * FROM `CLASS` WHERE `STREET_NUM`=\"$street_num\" AND `STREET`=\"$street\" AND `CLI`=$grid ORDER BY `APT`";
$quGetBPics = mysqli_query($dbh, $quStrGetBPics) or die(dieNice("Sorry, couldn't find Pictures for that Building at $street_num $street<BR>$quStrGetBPics<BR>$quGetBPics<BR>", "E-Building Pics"));


while ($rowGetBPics = mysqli_fetch_object($quGetBPics)) {

if ($rowGetBPics->CID > "0") {
...
<TD VALIGN="MIDDLE" ALIGN="CENTER">
&nbsp;&nbsp;&nbsp;<B><FONT SIZE="+1"><NOBR>MANAGE PICTURES FOR <?php echo $rowGetBPics->STREET_NUM ;?>  <?php echo $rowGetBPics->STREET ;?> # <?php echo $rowGetBPics->APT ;?></NOBR></FONT></B><BR>

<FONT SIZE="-1"><a href="<?php echo "$PHP_SELF?op=adlEdit&cid=$rowGetBPics->CID";?>" target="_<?php echo $rowGetBPics->CID ;?>">Listing # <?php echo $rowGetBPics->CID ;?></A></FONT><BR>
</TD></TR></TABLE>
...
<?php
      ...
      $quStrGetPics = "SELECT * FROM PICTURE WHERE CID=$rowGetBPics->CID ORDER BY PID";
...
<?php if ($user_level>0) {?>
<NOBR>
		<a href="<?php echo "$PHP_SELF?op=editPic&pid=$rowGetPics->PID&cid=$cid"; ?>"><img border="0" vspace="0" hspace="0" src="../images/icons/edit.gif">Edit</a> &nbsp;|&nbsp; <a href="<?php echo "$PHP_SELF?op=watermarkPic&pid=$rowGetPics->PID&ext=$rowGetPics->EXT&cid=$cid"; ?>">Watermark</a> &nbsp;|&nbsp; <input name="pid[]" type="checkbox" value="<?php echo $rowGetPics->PID;?>">Delete 


</NOBR><br><NOBR><B>Rotate:</B> <a href="<?php echo "$PHP_SELF?op=rotatePic&pid=$rowGetPics->PID&ext=$rowGetPics->EXT&cid=$cid&degrees=90"; ?>">90&deg; <img border="0" vspace="0" hspace="0" src="../images/icons/rotate-right.gif" ALT="rotate 90 degrees" HEIGHT="16" WIDTH="16" ALIGN="BOTTOM"></a> | <a href="<?php echo "$PHP_SELF?op=rotatePic&pid=$rowGetPics->PID&ext=$rowGetPics->EXT&cid=$cid&degrees=270"; ?>">270&deg; <img border="0" vspace="0" hspace="0" src="../images/icons/rotate-left.gif" ALT="rotate 270 degrees" HEIGHT="16" WIDTH="16" ALIGN="BOTTOM"></a> | <a href="<?php echo "$PHP_SELF?op=rotatePic&pid=$rowGetPics->PID&ext=$rowGetPics->EXT&cid=$cid&degrees=180"; ?>">180&deg;<img border="0" vspace="0" hspace="0" src="../images/icons/rotate-180.gif" ALT="rotate 180 degrees" HEIGHT="16" WIDTH="16" ALIGN="BOTTOM"></a></NOBR><FONT SIZE="-4"><BR></FONT>
<B>Picture Order:</B> 
<?php
...
&nbsp; <a href="<?php echo "$PHP_SELF?op=editPic&pid=$rowGetPics->PID&cid=$cid"; ?>">Change Order</a><BR>
<a href="javascript:popUpPicCode('<?php echo "./pages/picturelinks?pid=$rowGetPics->PID&ext=$rowGetPics->EXT&cid=$cid"; ?>');">HTML &amp; Links Picture Codes</A><br>
