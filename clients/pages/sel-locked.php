...
if((($rowGetAds->LOCKED==1) AND ($user_level>"4")) OR (($user_level>0) AND ($rowGetAds->LOCKED=="NULL"))){

			  echo "<a href='$PHP_SELF?op=deactivate&cid=$rowGetAds->CID&return_page_div=$k'>";}
			  echo "<img border='0' vspace='0' hspace='0' width='16' height='16' src='../assets/images/act.gif' alt='Activated Ad - Click to shut off the Ad' title='Activated Ad - Click to shut off the Ad'></a>";} else {
			  
			  if((($rowGetAds->LOCKED==1) AND ($user_level>"4")) OR (($user_level>0) AND ($rowGetAds->LOCKED=="NULL"))){
			  
			  echo "<a href='$PHP_SELF?op=activate&cid=$rowGetAds->CID'>";} echo "<img border='0' vspace='0' hspace='0' width='16' height='16' src='../assets/images/inact.jpg' alt='Deactivated Ad -  - Click to Advertise This Listing' title='Deactivated Ad - Click to Advertise This Listing'></a>";} ?>
...
<?php } ?><?php } ?></FONT>
		      ...
		      <?php if ($user_level >= 2 ) {
			  echo "<a href='$PHP_SELF?op=delete&cid=$rowGetAds->CID'><img border=\"0\" src=\"../images/icons/delete.gif\" alt=\"delete\" title=\"delete\" vspace=\"0\" hspace=\"0\"></a>";} ?>
...
<a href="<?php echo "$PHP_SELF?op=mail_listing&cid=$rowGetAds->CID";?>" target="_new"><IMG src=../images/icons/email.gif BORDER=0 HEIGHT=15 WIDTH=22 alt="Email Listing" title="Email Listing"></A>
        ...
        <?php if (($user_level>="3") or ($user_level<"3" AND $rowGetAds->STATUS=="ACT") ) { ?>
