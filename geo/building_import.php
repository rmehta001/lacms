...
$quStrAddr="SELECT * FROM CLASS INNER JOIN LOC ON CLASS.LOC=LOC.LOCID LIMIT 600";
...
while($rowAddr = mysqli_fetch_object($quReqAddr))
{
   ...
   echo $address."<br>";
   $quStrCacheLookup="SELECT * FROM buildings WHERE STREET_NUM='"
   			     .strtoupper($rowAddr->STREET_NUM)."' AND STREET='"
 			     .strtoupper($rowAddr->STREET)."' AND LOC='"
			     .$rowAddr->LOC."'";
      ...
      if($xml)
      {
	    ...
	    { echo "ERROR: Multiple Results"; }
	       ...
	       foreach($xml->result as $result)
	          ...
	          elseif($result->type=="street_address")
	          { 
...
//			echo $addr_comp->type;
			...
			{ $zip=$addr_comp->short_name; }
		     ...
		     $insStrBuildings="INSERT INTO buildings (STREET_NUM,STREET,CITY,STATE,ZIP,LAT,LON,LOC) "
	     			      ."VALUES (\"".strtoupper($rowAddr->STREET_NUM)."\",\""
				      .strtoupper($rowAddr->STREET)."\",\"".$city."\",\""
				      .$states[$rowAddr->STATE]."\",\"".$zip."\",\""
