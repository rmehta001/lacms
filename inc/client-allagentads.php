...
function format_ad_client ($thisRowGetAds, $thisDEFINED_VALUE_SETS) {
	...
	{	$thumb=get_thumb($thisRowGetAds); 	}
	else
	{	$thumb="No Photo Available";	}
	...
	if ($thisRowGetAds->AUTO_WRITE) {
		...
		$features_array['FEATURES_HIGH_SPEED_INTERNET'] = $thisRowGetAds->HIGH_SPEED_INTERNET        ;
...
//	$rooms_formated=preg_replace('/-/',' ',$rooms_formated);
	...
	$adString="<tr valign=top><td valign=middle align=center>$thumb&nbsp;<font size=1>$morePics</td><td><font size=2>$thisRowGetAds->LOCNAME - $noFee $rooms $baths $thisRowGetAds->BODY $amenity_string $feature_string $hasExternalPics $hasVirtualTour $hasYouTubeURL $isNew #$thisRowGetAds->CID&nbsp;&nbsp;$signature</td><td><font size=2>$fdate</td><td><font size=2>$price&nbsp;</td></tr>";
	return $adString;
}
?>
