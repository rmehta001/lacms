...
if ($recordset->LAST_CONTACT_ACTION == "8") { $LCA="- Other" ;}
		...
		$string = "<a href=".$_SERVER['PHP_SELF']."?op=editLandlord&lid=$recordset->LID>$recordset->HOME_NAME_FIRST $recordset->HOME_NAME_LAST, $recordset->OFF_NAME</A>";
...
$string .= "<A HREF=\"".$_SERVER['PHP_SELF']."?op=addendums&lid=$recordset->LID\" target=\"_newaddenda\">Additional Addenda & Docs</a><BR>";
...
function format_ad ($thisRowGetAds, $thisDEFINED_VALUE_SETS) {
	...
	$isNew = isset($isNewLog) ? "<img src='https://www.BostonApartments.com/images/newIcon.gif'>" : "";
...
$condofee =  ($thisRowGetAds->CONDO_FEES) ? "<B>Condo Fee:</B> \$".$thisRowGetAds->CONDO_FEES : "";} else {$condofee = "";}
	...
	$rooms = (isset($thisRowGetAds->ROOMS)&&$thisRowGetAds->ROOMS !="0.00") ? $thisDEFINED_VALUE_SETS['ROOMS'][$thisRowGetAds->ROOMS] . " - " : "";
	$baths = ($thisRowGetAds->BATH !="0.00") ? $thisDEFINED_VALUE_SETS['BATH'][$thisRowGetAds->BATH] . " - " : "";

	$pets = ($thisRowGetAds->PETSA !="0.00"&&!is_null($thisRowGetAds->PETSA)) ? $thisDEFINED_VALUE_SETS['PETSA'][$thisRowGetAds->PETSA] . " - " : "";


	$parkingnum = isset($thisDEFINED_VALUE_SETS['PARKING_NUM'][$thisRowGetAds->PARKING_NUM]) ? $thisDEFINED_VALUE_SETS['PARKING_NUM'][$thisRowGetAds->PARKING_NUM] . " parking " : "";

	$parkingtype = isset($thisDEFINED_VALUE_SETS['PARKING_TYPE'][$thisRowGetAds->PARKING_TYPE]) ? $thisDEFINED_VALUE_SETS['PARKING_TYPE'][$thisRowGetAds->PARKING_TYPE] . " " : "";


	$parkingcost = isset($thisDEFINED_VALUE_SETS['PARKING_COST'][$thisRowGetAds->PARKING_COST]) ? $thisDEFINED_VALUE_SETS['PARKING_COST'][$thisRowGetAds->PARKING_COST] . "\$$thisRowGetAds->PARKING_COST." : "";



	$laundry = isset($thisDEFINED_VALUE_SETS['LAUNDRY_ROOM'][$thisRowGetAds->LAUNDRY_ROOM]) ? $thisDEFINED_VALUE_SETS['LAUNDRY_ROOM'][$thisRowGetAds->LAUNDRY_ROOM] . " - " : "";




	if (isset($thisRowGetAds->AUTO_WRITE)) {
	...
	//	$features_array['FEATURES_WALK_IN_CLOSET']      = $thisRowGetAds->FEATURES_WALK_IN_CLOSET    ;
		...
		foreach ($features_array as $name => $value) {
			if ($value) {
				...
				$feature_string .= " - ";
				echo $thisDEFINED_VALUE_SETS[$name][$value];// ?? "";
			}
...
//		$amenities_array['AMENITIES_ELEVATOR_BUILDING']  = $thisRowGetAds->AMENITIES_ELEVATOR_BUILDING;
	...
	if (($thisRowGetAds->MAP) AND ($thisRowGetAds->CLI != 1075)) {
        ...
        if($thisRowGetAds->AGENCY_HEADERS)
        {
                ...
                $quAgency = mysqli_query($GLOBALS['dbh'],$strAgency);
...
function format_ad_ft ($thisRowGetAds, $thisDEFINED_VALUE_SETS, $ftSearchTerm) {
	...
	$isNew = ($isNewLog==1) ? "<img src='https://www.BostonApartments.com/images/newIcon.gif'>" : "";
	$hasPics = ($thisRowGetAds->PIC!=0) ? "<a href=\"https://www.BostonApartments.com/homepage.php?cli=$thisRowGetAds->CLI&ad=$thisRowGetAds->CID\" target=\"blank\"><img border='0' width='22' height='12' src='https://www.BostonApartments.com/images/pic.gif'></a>" : "";

	$hasExternalPics = ($thisRowGetAds->EXTERNALPIC) ? "<A HREF=\"https://www.BostonApartments.com/homepage-MLS.php?cli=1075&ad=$thisRowGetAds->CID\"><img border='0' width='22' height='12' src='https://www.BostonApartments.com/images/pic.gif'></a>" : "";
	
	$hasVirtualTour = ($thisRowGetAds->VIRT_TOUR) ? "<a href=\"$thisRowGetAds->VIRT_TOUR\" target=\"blank\"><img border='0' width='59' height='11' src='https://www.BostonApartments.com/images/virtualtour.gif'></a>" : "";

// The UI of the fee function
	if ($thisRowGetAds->NOFEE==3 || $thisRowGetAds->NOFEE==8 || $thisRowGetAds->NOFEE==9) {
		$noFee = "";
	}else {
		$noFee = ($thisRowGetAds->NOFEE) ? "<font color='red'><b>" . $thisDEFINED_VALUE_SETS['NOFEE'][$thisRowGetAds->NOFEE] . "</b></font> - " : "";
...
// the UI of assessment
if ($thisRowGetAds->ASSESSMENT !="0.00"){
$assessment =  ($thisRowGetAds->ASSESSMENT) ? "<B>Assessed Value:</B> \$".$thisRowGetAds->ASSESSMENT : "";} else {$assessment = "";}
// the UI of propertytax
if ($thisRowGetAds->PROPERTYTAX !="0.00"){
$propertytax =  ($thisRowGetAds->PROPERTYTAX) ? "<B>Property Tax:</B> \$".$thisRowGetAds->PROPERTYTAX : "";} else {$propertytax = "";}
// the UI of condo fees
if ($thisRowGetAds->CONDO_FEES !="0.00"){
$condofee =  ($thisRowGetAds->CONDO_FEES) ? "<B>Condo Fee:</B> \$".$thisRowGetAds->CONDO_FEES : "";} else {$condofee = "";}
...
// rooms and baths		
    ...
    $body="";
	//$body = preg_replace ($ftSearchTerm, "<span style=\"background-color: #FFFF00\">$ftSearchTerm</span>", $thisRowGetAds->BODY);
...
//This is one of the functions of admin part/ rewired by wei
	...
	function __construct ($checkBoxName, $keyName, $keyDataType, $keyOperation, $keyValue)
		{
...
class PwFile{
    ...
    function __construct($filename) // constructor
    {
...
function dieNice ($code, $dMsg) {
	...
	$op = $_GET['op'];
	$quStrRecordUSA = "INSERT INTO USA (SIDNUM, REQNUM, OP, DELTA, STATUS, MSG) VALUES ('" . $_SESSION['usa_sid_num'] . "', '" . $_SESSION['usa_req_num'] . "', '" .$op. "', '$usa_req_delta', '1', '$dMsg')";
	$quRecordUSA = mysqli_query($GLOBALS['dbh'], $quStrRecordUSA); 
	//usa end//
?>
...
function getGroupInfo ($group) {
	...
	$quGetGroupInfo = $GLOBALS['dbh']-> query($quStrGetGroupInfo);
	...
	$quGetGroupCount = mysqli_query($GLOBALS['dbh'],$quStrGetGroupCount);
	$quGetGroupActCount = mysqli_query($GLOBALS['dbh'],$quStrGetGroupActCount);
...
function make_map ($rowGetAd) {
	...
	$quGetMap = mysqli_query($GLOBALS['dbh'], $quStrGetMap) or die (mysqli_error($GLOBALS['dbh']));
...
// wei rewrited the function
function make_map_email ($rowGetAd) {

	$quStrGetMap = "select * from MAP_OFFER where id='$rowGetAd->MAP'";
	$quGetMap = mysqli_query($GLOBALS['dbh'],$quStrGetMap) or die (mysqli_error());
	$rowMapDetails = mysqli_fetch_object($quGetMap);
...
$totalrooms =  ($thisRowGetAds->TOTAL_NUM_ROOMS) ? " - ".$thisRowGetAds->TOTAL_NUM_ROOMS." Room " : "";
 ...
 elseif (($thisRowGetAds->TOTAL_NUM_ROOMS !="0") AND ($thisRowGetAds->TOTAL_NUM_ROOMS !="1")){
$totalrooms =  ($thisRowGetAds->TOTAL_NUM_ROOMS) ? " - ".$thisRowGetAds->TOTAL_NUM_ROOMS." Rooms" : "";} 
else {$totalrooms = "";}

if ($thisRowGetAds->SQFT !="0"){
$sqft =  ($thisRowGetAds->SQFT) ? " - ".$thisRowGetAds->SQFT." SqFt." : "";} 
