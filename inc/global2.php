...
$avail = "<BR>Available: <B> $avail_month/$avail_day/$avail_year</B>. ";
	...
	if ($thisRowGetAds->AUTO_WRITE) {
		...
		if ($feature_string) {
			$feature_string = " <strong>Features:<BR></strong> " . $feature_string;
		}
...
$buildingstyle = ($thisRowGetAds->BUILDING_STYLE) ? $thisDEFINED_VALUE_SETS['BUILDING_STYLE'][$thisRowGetAds->BUILDING_STYLE] . " " : "";
	...
	$user_sig = ($thisRowGetAds->USE_USER_SIG) ? " $thisRowGetAds->USER_SIG " : "";
	$refNum = "$thisRowGetAds->ABV-$thisRowGetAds->CID";
	$adString =  "<HR>\n<B>$thisRowGetAds->LOCNAME - $noFee$status_sale</B> $priceprefix$price$pricesuffix $avail<BR>$rooms$baths$totalrooms$sqft $floor $buildingstyle $buildingtype <BR><BR>$thisRowGetAds->BODY <BR>$feature_string <BR>$amenity_string$laundry $pets $parkingnum$parkingtype $parkingcost<BR>$price_sqft  $condofee $assessment $propertytax<BR><BR>$user_sig <BR>AD#$refNum 
...
function get_thumb ($thisRowGetAds)
{
   ...
   {
      $rowGetPics="<a href=\"http://www.bostonapartments.com/homepage.php?cli=$thisRowGetAds->CLI&ad=$thisRowGetAds->CID\" rel=\"gb_page_fs[]\"><img border=0 src='http://www.bostonapartments.com/pics/$thisRowGetAds->THUMBNAIL' width='125' alt='$rowGetPics->DESCRIPT'></a><br>";
   } else {
      ...
      {
         $rowGetPics="<a href=\"http://www.bostonapartments.com/homepage.php?cli=$thisRowGetAds->CLI&ad=$thisRowGetAds->CID\" rel=\"gb_page_fs[]\"><img border=0 src='http://www.bostonapartments.com/pics/$rowGetPics->PID.$rowGetPics->EXT' width='125' alt='$rowGetPics->DESCRIPT'></a><br>"; 
      }
   }
   return $rowGetPics;
}
